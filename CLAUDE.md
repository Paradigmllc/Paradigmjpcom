...
- 🆕 **[2026-05-20] 営業フロー統合 (カルテ→診断→④フォーム営業→進捗・自己完結・本番稼働)**: 監査で営業OSの二重化を発見 (本番=Appexxme `leads`(198)/`proposal_pages`(173)・paradigm-HP=`sales_companies`(7デモ)・同 Supabase 共有) → ユーザー方針「**paradigm-HP 自己完結**」確定 (memory `project-sales-os-duplication.md`)。実装: ① `lib/sales/outreach/*` (BrowserProvider 抽象 + state-machine + form-classifier + preflight + orchestrator) + `/api/sales/outreach/run` (dryRun 既定=安全) ② **HttpFormProvider** = ブラウザ不要 HTTP 送信 (サーバー増設なし・標準フォーム CF7/WPForms/素POST対応)・`worker/` (Playwright Stealth×Crawlee) は SPA 専用の将来オプション (managed CDP・Droplet に Chromium 置かない方針) ③ `lib/sales/sources/form-discovery.ts` (regex→sitemap→heuristic→LLM→SPA の5層) + enrich 配線 → `meta.contact_form_url` ④ **重複排除** `lib/sales/dedup.ts` + migration_006 (canonical domain=hard UNIQUE + name_key=soft + `findExistingCompany` 三段照合)・本番実証 (3行→batch_dupes_removed=1) ⑤ **LiteLLM 風フォールバック** `lib/deepseek.ts` (DeepSeek V4試行→空/失敗で deepseek-chat→OpenRouter・`DEEPSEEK_API_BASE` で LiteLLM proxy 切替)・旧 V4 PRO 永久指定は撤回 (v4系は実APIで空応答) ⑥ **DeepSeek 作り込み自動化** `personalize.ts autoPersonalize` (enrich 完了で fire-and-forget 発火・「寄せ集め禁止」prompt 強化・diagnostic.ts が `meta.personalized_copy` 優先採用) ⑦ **Notion CSV→作成+enrich** (sync-companies-from-notion 2経路化) + pg_cron jobid3(companies 5min)/jobid4(templates 15min)・テンプレ 56 を Notion テンプレDB に push ⑧ `lib/sales/kpi.ts` + `/api/sales/kpi-snapshot`。tests 92/92・tsc clean・本番 E2E 稼働確認 (commits 06ff74d→7228a3d)。**残**: Payload paradigm-tables 0 (別領域・並行セッション) / レポートデザイン刷新 (DiagnosticReport.tsx 並行編集中で不触) / 実リード投入。Notion URL: 営業OS `35fa2b78f3fc81299d91e457889ee393` / リードDB `8cbab1f5...` / テンプレDB `115e2b0e...`
- 🆕 **[2026-05-21] Notion 即時 GUI 化 (Notion編集→即Supabase・Webhook駆動)**: ユーザー方針「Supabase=SSOT 維持・GUI は Notion で完結・編集は即 Supabase 反映・全フィールド編集可」確定。旧 5min cron (sync-*-from-notion) を **Notion API Webhook 主経路** に置換 (cron は取りこぼし保険で残置)。実装: ① `/api/sales/notion-webhook` 新規 = 検証ハンドシェイク (verification_token→`NOTION_WEBHOOK_SECRET`) + 署名検証 (X-Notion-Signature = HMAC-SHA256(raw body)・timing-safe・fail-closed) + **エコー防止** (`notionGetBotId` で自分の変更=Supabase→Notion 由来は無視) ② `lib/sales/notion-apply.ts` = 親 DB id→4テーブル振り分け (`normId` で dashed UUID 正規化照合) + 1ページ即反映。**識別子(id/region/domain/slug/name_key/notion_page_id) とシステム計測値(pagespeed/report_views/is_hot_lead 等)はサーバー側で書込無視=ロック**・人間管理フィールドは全反映。リード新規行は domain あれば作成+enrich 発火 ③ `lib/sales/notion-webhook-verify.ts` = HMAC 純関数 (test 9件) ④ `lib/notion.ts` に `notionGetPage`/`notionGetBotId` + `extractProperty` を **status型(Kanban)・people型(担当者)** 対応 ⑤ `.env.example` に `NOTION_WEBHOOK_SECRET`/`NOTION_BOT_ID`/region別 DB id override 追記。反映速度=数秒〜最大1分 (Notion aggregation 仕様・真の即時は不可)。tests 101/101 (92+9)・自分のファイル tsc clean。**手動手順 (自動化不可)**: integration Webhooks タブで subscription 作成→検証 token を env 保存+UI 貼り戻し→再 deploy。**残**: Notion DB views UX (Kanban/HOT/Calendar) 整備
- 🆕 **[2026-05-21] PayloadCMS 管理画面 全面拡張** (ユーザー指示「機能少なすぎる」→4領域+RLS): ① **ナビ DB 化** Header/Footer global 新設 (navItems+dropdown/cta/columns/social/legal/copyright・`makeGlobalAutoTranslateHook` path-key 再帰で ja→11locale 自動翻訳) → `lib/navigation.ts` reader → SiteHeader/SiteFooter/ConditionalSiteChrome 配線 (未設定時 i18n 既定 fallback で非破壊)。旧ハードコード NAV 廃止 (A-CONTENT/AE-PHP-7 準拠) ② **新コレクション3** TeamMembers(About 用) / Testimonials(掲載許諾 consent 必須・誠実化準拠) / Categories(blog taxonomy・Posts.categoryRef 優先採用) ③ **Settings 強化** seo既定値(OG/favicon depth1 populate) / tracking(GTM・GA4・Meta Pixel・headScripts/bodyScripts は `isAdminField` で admin限定=XSS 保護) / announcementバー(layout 注入・header top-9 offset) / company法的情報 ④ **ダッシュボード** `components/admin/BeforeDashboard.tsx` (件数/リードpipeline/監査6/新規作成) + beforeDashboard 登録 ⑤ **RLS** `migration_007_rls_paradigm.sql` DO-block 冪等 → paradigm 134/134 ENABLE 確認 (owner=payload_user + service_role bypass で Payload 無影響・anon deny=最小権限)。⚠️ deploy 後の新 table に RLS 再実行要・public schema の 60 rls_disabled は他PJ所有(s10-7)で不触 / tsc clean(残=既存.next stale) + vitest 101/101 ✅
