...
- 🚨 **[2026-05-13] DeepSeek V4 PRO 永久指定**: ユーザー強制 ("v3 ではなく V4 PRO・間違えないで・永久保存"). `src/lib/deepseek.ts` の `DEFAULT_MODEL = "deepseek-v4-pro"` で全 LLM 呼び出しを PRO 固定 (Flash は opts.model 明示時のみ). レガシー `deepseek-chat` / `deepseek-reasoner` / v3 系は新規禁止. グローバル CLAUDE.md NN ルール + メモリ feedback_important_rules.md に永久保存
