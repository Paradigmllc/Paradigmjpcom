...
- Hardened production startup with a container-local, fail-closed migration path that applies and verifies the 320-scenario catalog through the internal Postgres network before the application starts. This removes external SSH availability as a single point of failure while preserving the canonical release migration and all RLS/content gates.
