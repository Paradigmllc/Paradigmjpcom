...
- The corrected official-sampler one-shot canary generated a real 576x1024/73-frame pet motion clip: the dog and cat remain present at 0/25/50/75/100%, with restrained head, gaze and breathing motion instead of the prior brown-frame collapse. The previous worst-frame 512px SSIM rule incorrectly rejected this valid motion (0.490357), so the gate is being replaced with opening-detail SSIM plus low-frequency temporal structure and normalized luma-entropy preservation. Against the actual artifacts, the motion-aware score is 0.950736 for the corrected canary and 0.350389 for the collapsed output at the unchanged 0.78 threshold.
- ACTIVE HANDOFF: finish/release the non-billable one-shot job, merge and deploy the motion-aware fidelity gate, then rerender one canary through the deployed gate and verify the generated clip is selected rather than the supplied-photo fallback. Do not enable `PET_MOVIE_GPU_RENDER_ENABLED=true` until a five-shot film using distinct source photos also passes automated QA and both human reviews, and rights-cleared emotional sound design plus duration/photo-count policy are complete. No customer charge or delivery is authorized for QA.
