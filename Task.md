...
- ACTIVE HANDOFF: merge/deploy the least-privilege migration and Shopify implied-scope corrections, then re-run the authenticated production launch audit and production grant read-back. BASE Developers/OAuth, real products, payments, checkout, policies, and supported social credentials remain blocked. Commercial launch stays password-protected until all evidence exists.
