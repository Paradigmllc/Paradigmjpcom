## CURRENT STATUS - 2026-07-21 `/work` 500件キュー・Twenty解析保存・企業別文面・10章戦略レポート（本番実企業read-back完了 / 外部送信0）

- `/work`を入力元、Twentyを解析結果の出力先として固定した。日本企業と取得不能行を除く解析済み海外企業は、フォーム・文面・Japan Entry fitが要確認でもTwentyへ未送信の分析レコードとしてupsertする。既存Twenty同domainは重複作成せず同一会社を更新し、会社名、国、業種、SMB/Japan Fit、レポートURL、フォームURL、初回文面全文、要確認理由、Next ActionをPATCH後に再GETして完全一致した場合だけ`synced`とする。営業statusはTwentyの有効option `手動確認 / 未対応`、準備度はData Statusへ分離した。メール・フォーム・SNS等の自動送信経路はなく、DB制約と本番read-backで`sent=false`を維持する。
- 1回の入力上限を20件から**500件**へ拡張した。`manual_japan_entry_batches` / `manual_japan_entry_batch_items`にRLS、所有者境界、URL順序、lease、attempt、進捗、個別エラー、work IDを永続化し、明示操作で最大3件ずつdrainする。リロード・API timeout後も未完了itemを再開でき、cron・常駐polling・既存Revenue OS pipelineは使用しない。画面は500件全処理を進捗表示しつつ、DOM肥大化を避けるため履歴カードは先頭60件だけを表示する。
- 初回文面はDeepSeek V4 Proで企業別strategyを先に作り、公開ページ上の商品名・具体機能、監査済みJapan customer-path、選択したangle、国別のビジネストーン、許可/転送CTAを会社ごとに統合する。本文は**4段落・120〜190語**、生成目標145〜165語、挨拶`Hello {Company} team,`と`Tomohiro H / Paradigm LLC / contact@paradigmjp.com`署名を固定し、URL、domain、出典、添付、提携表現、初回価格、根拠外の需要・売上・機会損失・因果を決定論で拒否する。語数不足時は実測語数と必要rangeをDeepSeekへ返し、最大3回のbounded repairを行う。92/100かつ4軸23/25以上、安全性100、根拠・独自性gate通過時だけ本文を保存する。
- 顧客向け公開レポートは`manual_japan_entry_strategy_v4 / customer_japan_entry_opportunity_report`へ刷新した。Executive decision、商品診断、日本機会仮説、customer journey、positioning/localization、commercial model、GTM、operating model、90/180日roadmap/KPI、risk/recommendationの**順序固定10章**、各章の根拠区分・行動・KPI・decision gate、推定値の根拠/限界を持つ。旧汎用テンプレートを使わず`legacyTemplateUsed=false / automaticSendAllowed=false`を保存し、最低1,200語を契約で検査する。
- 初回本番実行で、Twenty selectに未登録値を送った不具合、`altairis.fr`が`www`へ301後にsecondary auditを空扱いする不具合、文面候補が120語未満のため2回修正後に全破棄される不具合を順に再現した。PR **#474 / #475 / #476**で、有効option固定、最終canonical URLと取得済みhomepage HTMLを監査証拠として再利用、実測語数付き3回目repairを実装した。短文を無条件採用するfallback、架空フォーム、架空根拠は追加していない。
- PR **#473**（500件/全解析Twenty/V4レポート）、**#474**（Twenty select）、**#475**（canonical audit）、**#476**（語数repair）をmain **51f63d7e**まで統合した。正式deploymentは順に`zi58soy3vfgchs5pkfmj5m2c` / `u4tgj4zg83xzcm73viovd5ww` / `a3gq1tu6zyx9cw9w9hm7c8sm` / `rnc9sq7yqbfzeqjx5vdav0f6`で、最終`npm run release:prod`はDB **93/93**、Quality Guard **0 errors / 76 existing warnings**、Sales health JSON `ok:true`、Twenty HTTP 200・worker restart 0、Realtime、Traefik、RLS、zero-sendを含むpost-deploy gateを完走した。ローカルは全Vitest **224 files / 1,026 tests**、TypeScript、production build **408/408 pages**がpass。
- 本番同一履歴`altairis.fr`（work `086f9c57-5118-4b88-b326-5af3d632be49`）を4回目として実再解析し、`stage=complete / status=needs_review`（Japan Entry fitの人間確認であり処理失敗ではない）、実フォーム`https://www.altairis.fr/contact/`、企業固有文面**951文字・本文127語・4段落**、quality **92/100**、safety **100/100**、issues 0、URL混入0、正式署名一致を別GETでread-backした。V4レポートは**10章・3,127語**、公開URL HTTP 200、`legacyTemplateUsed=false / automaticSendAllowed=false`。Twenty会社`2e21acdf-c028-403d-9965-b371cd54d30c`を直接APIで読み戻し、会社/フォーム/レポート/status/Next Actionと初回文面全文が一致、summary 1,445文字、`never sent automatically`を確認した。`sent=false`、外部送信0件。

## CURRENT STATUS - 2026-07-20 `/work` SPA企業証拠・完全オーダーメイド文面・顧客向けレポートV3（本番release・同一履歴read-back完了 / 外部送信0）
...
- 実DeepSeek公式APIでScreenshot to Codeの描画後相当証拠を投入し、商品固有の`Screenshot to Code / screenshots and videos / React・Vue等`、公開ページ上の日本語導線未確認、会社名入りrouting CTA、Tomohiro H / Paradigm LLC / contact@paradigmjp.com署名を含む本文を生成。品質 **92/100（4軸23/25）**、安全性 **100/100**、独自性 **97/100**、URL/ドメイン・出典・商業条件0を確認した。TypeScript、対象ESLint、Quality Guard **0 errors / 76 existing warnings**、全Vitest **220 files / 993 tests**がpass。本番release、同一履歴の再解析、DB/UI/公開V3レポート/Twenty read-backは次工程。フォーム・メール・SNS等の外部送信経路は追加していない。
- PR **#464** / main **75693dc8** / deployment **lnrwzt5uvbixfeom39x8qqgo**を正式releaseし、本番同一履歴で`evidenceMode=browser_rendered`、実商品説明、フォーム`no_public_form / 14 routes / form_url=NULL`、顧客V3レポート、Twenty `skipped / company_id=NULL / sent=false`をread-backした。その際、SPAの販促見出し`Build User Interfaces 10x Faster`が補助根拠に選ばれ、未許可数値`10`で文面保存をfail-closeしたことを検出。PR **#465** / main **b6376967** / deployment **nszkjks759jdi7dopfjx87iw**で数値・販促見出し除外をreleaseしたが、本番再解析ではDeepSeek criticが最終質問の企業名を誤読して90/100で停止した。そこで最終質問そのものに企業/製品anchorを必須化し、監査済みcustomer-pathを同じ最終段落へ保持する明示contract、決定論gate通過項目をcriticが再否定しない校正契約、根拠説明文優先rank、一般化した日本ユーザー行の削除指示をPR **#466** / main **b52b50d7**へ追加した。実サイト相当のDeepSeek smokeは連続 **2/2**、全Vitest **220 files / 993 tests**、Quality Guard **0 errors / 76 existing warnings**がpass。deployment **pmo6mzz3mu3tgms9dkmaf9qn**を正式release後、履歴ID `40c23b3b-fa0e-4540-9f1d-09082d67bb93`を11回目として再解析し、別GETで本文永続化、`generation_status=passed / quality=92（4軸23）/ safety=100 / uniqueness=100`、実商品機能、会社名入り最終質問、正式署名、URL・出典・商業条件・`10x`なしをread-backした。顧客V3レポートはHTTP 200・内部ラベル0。フォームは実サイト上で`no_public_form / 14 routes / form_url=NULL`、国・SMB・実フォーム未確認のためTwentyは正しく`skipped / company_id=NULL / sent=false`。現在の`/work`履歴はこの1件のみで、適格企業のTwenty実同期件数はまだ0件と明記する。外部送信0。
...
- 空本文・明示404を`empty_or_soft_404`としてmissing判定し、manual workflowの最終保存ゲートを`verification=form / confidence>=90 / inspection.status=form / email+message+submit`へ統一した。未検証候補は内部の探索証跡だけに残し、`form_url`・レポートcontactRoute URL・master ledger URL・observed evidenceには出さない。DeepSeek生成はフォーム有無と独立して継続し、`generation_status/error/usage/generated_at`をDB保存、文面なしも画面で理由表示、成果イベント未記録の`needs_review`は同じ履歴を再解析できる。
- idempotent data migrationで既存の誤フォームURLを除去し、過去の文面未生成行を`retry_required`へ正直に補正した。release gateは不正なform URL、レポート/台帳URL不一致、文面失敗理由欠落、`sent=true`をpost-deploy DBで0件検査する。PR **#451** / main **83f03028** / deployment **h9ydf8d9911nxzwj3yn1kme9**で正式releaseを完走した。
- 本番の同一履歴ID `40c23b3b-fa0e-4540-9f1d-09082d67bb93`を実再解析し、現行read-backは`needs_review / complete / attempts=4`、文面 **987文字**、DeepSeek `generation_status=passed / editorial=true / quality=94 / safety=100 / uniqueness=100`。本文はURL・domain・出典表現なし。`form_url=NULL`、V2レポートcontact URL `NULL/missing`、master ledger contact URL `NULL`、画面フォームボタン0、公開レポート内contact link 0。国・SMB・実フォームが未確認のためTwentyは`skipped`が正しく、Twenty DBの同domain **0社**、`twenty_company_id=NULL / sent=false / 成果timestamp全null`を維持する。
- 本番Chrome追試で、履歴日時のlocale差によるReact hydration #418と、`Japan market public-page audit`等の内部ラベルをURLとして扱うconsole errorを検出した。日時をJSTの決定論formatへ変更し、HTTPS形だけをURL検証する。PR **#452** / main **54c5541c** / deployment **y9sstyaldpyjixypuwqr2qey**で再releaseし、最新container **n8i2sjiqvr2d8hrzppop2m2i-215347744253**はmain imageでhealthy。リロード後の`/work`・公開V2レポートは新規console error/warn **0**、横overflow 0、文面表示、フォームボタン0、レポートcontact link 0。
- 検証はTypeScript、対象ESLint、Quality Guard **0 errors / 75 existing warnings**、重点Vitest **6 files / 22 tests**＋console追試 **2 files / 4 tests**、全Vitest **213 files / 962 tests**、production build **408/408 pages**、DB **91/91**、Sales health JSON `ok:true`、Twenty HTTP 200、worker restart 0、Realtime/Traefik/RLS/zero-sendを含む最終`release gate passed`。既存automationとは分離し、メール・フォーム・SNS等の外部送信経路は追加していない。

## CURRENT STATUS - 2026-07-19 `/work`診断レポートV2完全分離（本番release・PC/mobile実画面QA完了 / 外部送信0）
...
- PR **#444** / main **63d82fea**を統合し、正式`npm run release:prod`のdeployment **jx6cfnj6bkngkifbn2whnwij**でV2 migration、DB **91/91**、公開V2レポートHTTP 200、Sales health JSON `ok:true`、Twenty HTTP 200、worker restart 0、RLS/zero-send guardを通過した。本番DB read-backは既存1件が`manual_japan_entry_v2 / legacyTemplateUsed=false / automaticSendAllowed=false / sent=false`で、旧キーなし。
- 初回release後のPC/Pixel 7 full-page撮影で、V2本体は正常だが`/work-report`が独立ページ判定に入らず、外側にサイト共通の市場訴求帯・header/footer・chatbotが混入することを検出した。`ConditionalSiteChrome`のstandalone route SSOTへ`/{locale}/work-report/*`を追加し、通常ページを巻き込まないroute単体テストとrelease gateを追加した。PR **#445** / main **747dda1a**を統合し、正式`npm run release:prod`のdeployment **w5p9vdhlu69mo0u5d5g2q49h**でDB **91/91**、公開V2レポート、Sales health JSON `ok:true`、Twenty HTTP 200、worker restart 0、RLS/zero-sendを含む`release gate passed`を確認した。PC 1440px / mobile 412pxの本番Chrome監査は横overflow 0、console/page error 0、旧市場訴求・Cookie UI 0で、専用レポートだけを表示。本番DB read-backは`manual_japan_entry_v2 / legacyTemplateUsed=false / automaticSendAllowed=false / sent=false / 旧content_templateなし`。現行のScreenshot to Codeは根拠ゲート未通過のため`initial_message=false / Twenty skipped`を維持し、不適格レコードを保存済みに見せかけない。TypeScript、対象ESLint、Quality Guard **0 errors / 75 existing warnings**、release-doctor local static gate、重点Vitest **3 files / 10 tests**、全Vitest **212 files / 955 tests**、production build **408/408 pages**がpass。海外SMB限定、公開ページ根拠、初回文面のURL/出典禁止、Twenty未送信保存、人間確認、自動送信0を維持する。
...
- PR **#392** をmainへ統合し、正式 `npm run release:prod` のdeployment **r43ycytmrns4e440ns2jpybz** を完走。DB **91/91**、post-deploy公開smoke、Twenty HTTP 200、Realtime、Sales health `ok:true`、未認証候補投入API **401** をread-backした。生成ビジュアルAPIはHTTP **200 / image/svg+xml / 2,615 bytes**、既存demo hostはHTTP **200**。実候補の生成投入・Twenty書込み・外部送信は引き続き0件。
...
- `paradigmOutreachTargetUrl` はTwenty metadata上で `LINKS` / 表示名 `営業先URL`、一覧ビュー・record viewともposition 9 / visible true。正式 `npm run release:prod` は deployment `uk6ad2trdzmuof1c0yzfvy4s`、DB 89/89、Twenty worker、Realtime、公開smoke、Sales health `ok:true` を含む `release gate passed`。
...
## CURRENT STATUS - 2026-07-16 Premium V3 DEMO 全体底上げ（共通UI + コピー正規化 / 本番release・公開DEMO再確認完了）
...
- Campaign copy states that eligibility is limited to the first 10 selected launch partners and confirmed in writing. The same terms are synchronized across the English homepage, contact application, FAQ, terms, refund policy, About, manifest, JSON-LD, chatbot fallback/knowledge, and CMS seed blocks. No Japanese domestic Japan Entry page was changed.
- Verification completed: JSON parse, targeted Vitest **5 files / 33 tests**, TypeScript `tsc --noEmit`, Quality Guard **0 errors / existing warnings**, `npm run build` compiled successfully and generated **408/408 pages**, `git diff --check` passed. Existing user change `src/lib/sales/twenty-sync-list-lead.test.ts` remains unstaged and untouched.
- PR **#364** (campaign) and **#367** (release smoke marker) were merged; formal `npm run release:prod` completed with deployment **zemr1vh32a5fyscrsw3g9jux**. Release pre/post gates passed: DB **89/89**, CMS English homepage + 22 English Japan Entry articles, Traefik/Cloudflare origin lock, Realtime, Twenty worker restart 0, and Sales health JSON `ok:true`.
- Public read-back on 2026-07-16 returned HTTP **200** and the campaign strings (`$12,000`, `$2,000/month`, `$0/month`, `first 10 selected launch partners`) on `/en`, `/en/pricing`, `/en/package`, `/en/faq`, `/en/terms`, `/en/refund`, and `/en/contact`; `/en/pricing` and `/en/package` both expose the `launch-partner-campaign` diagram anchor. The original Twenty test change remains restored and unstaged.
## CURRENT STATUS - 2026-07-16 Managed Japan Desk公開コピー・記事アップグレード（本番release・read-back完了 / 外部送信0）

- 添付のJapan Entry Package整理に合わせ、英語公開面の商用説明を確認済み条件へ統一した。標準Managed Japan Deskは`$2,000/month`、最初の10社に選定されたlaunch partnerは1〜6か月が`$0/month`、7か月目以降は契約条件に基づく`$2,000/month`。未確認のデポジット、支払手段、成果保証、広告費込み、追加の希少性は公開コピーへ追加していない。
...
## CURRENT STATUS - 2026-07-20 公開DEMO実レンダラー品質修正（本番read-back完了 / 外部送信0）
