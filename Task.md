...
- Direct Shopify Admin access is verified. The store is named `SERICIA`; `Tableware`, `Craft`, `Living`, and `Gifts` collections exist; and the custom domain resolves to Shopify.
- The unpublished `SERICIA WoodMart OS2 - Development` theme (ID `144315482160`) contains the verified OS2 storefront and collection links. The live Rise theme and password protection remain unchanged. The supplied WoodMart OS1 package remains visual-reference-only due to 302 Theme Check errors.
- The installed `SERICIA BASE Sync` merchant app uses minimal product, inventory, and location scopes plus Shopify's 24-hour client-credentials flow.
- BASE OAuth, encrypted token storage, paginated item import, draft-only Shopify `productSet` upsert, inventory mapping, dry-run/apply UI, history, error visibility, notifications, and service-role-only RLS tables are implemented. Product deletion and automatic publishing are disabled.
- Production migration `20260802153000_shopify_base_sync.sql` is applied and verified. Deployment remains blocked on BASE Developers credentials and Coolify secret API reachability; do not request Shopify credentials from the user. After secure propagation, deploy, complete BASE OAuth, dry-run, apply draft sync, and verify product/price/inventory read-back.
- Keep Markets beyond Japan, shipping rates, and tax registrations unchanged until product weights, service levels, target countries, and tax-liability facts are confirmed.
