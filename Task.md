...
- Duplicated the live theme without modifying it and created the unpublished working theme `SERICIA WoodMart OS2 - Development` (theme ID `144315482160`). Shopify CLI authentication is complete and the development theme is synchronized under `shopify/theme`.
...
- Renamed the Shopify store to `SERICIA`, created the `Tableware`, `Craft`, `Living`, and `Gifts` collections, connected the unpublished homepage category cards to their collection handles, and pushed the verified theme update. The live Rise theme and storefront password remain unchanged.
- Created and installed the merchant-owned Dev Dashboard app `SERICIA BASE Sync`, released version `sericia-base-sync-2`, and limited it to `read_products`, `write_products`, `read_inventory`, `write_inventory`, and `read_locations`. The implementation now uses Shopify's 24-hour client-credentials token flow instead of a deprecated static token.
- Implemented BASE OAuth (`read_users read_items`), encrypted refresh/access token storage, BASE item pagination, normalization, HTML escaping, image allowlisting, four-collection classification, Shopify `productSet` draft upsert, inventory/location mapping, dry-run preview, admin UI, history, error visibility, DB bell + Slack notifications, service-role-only RLS tables, and production migration wiring. Product deletion and automatic publishing are intentionally disabled.
- Applied `20260802153000_shopify_base_sync.sql` to production and read back all three service-role-only tables. Active handoff: deployment is blocked until the BASE Developers client ID/secret exists for the actual BASE shop and the local runner can reach the approved Coolify secret API again. The Shopify client ID is stored in the approved local secret reference; the client secret remains available in Dev Dashboard but still needs secure propagation because every Coolify API attempt returned `fetch failed` and the browser connection reset during secret capture. Do not request the Shopify credentials from the user. After secret propagation, deploy, complete BASE OAuth, run dry-run, then apply draft sync and verify product/price/inventory read-back. Keep Markets beyond Japan, shipping rates, and tax registrations unchanged until product weights, service levels, target countries, and tax-liability facts are confirmed.
