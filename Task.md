...
- 対象Vitestはこの端末のworker起動タイムアウトで再実行が不安定だったが、変更ファイルESLintと`git diff --check`は通過。正式`npm run release:prod`はDB **91/91**、Coolify deployment **y2iax09vnfl5dqpchaiye0h3**、公開smoke、Sales health `ok:true`、Twenty/Realtime/Traefik、zero-send gateを含む`release gate passed`。公開`/cafe-sosomu`をread-backし、実DOMの`<img>`はowned JPEG（hero / pour-over / exterior）各3箇所、生成ビジュアルは不足分の6箇所、`data-hospitality-composition="1"`、split hero、内部権利文言0件。Hero原本はHTTP **200 / image/jpeg / 485,223 bytes**。外部送信・Twenty変更は0件。
