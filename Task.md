...
- PRs #701, #702, and #704 are merged. Production main `401ad9d6a24d927f8d60d3f8b9bbe9c1a83a7767` is running healthy through Coolify deployment `g347hblazx0r5727im2aj81f`; the one-time pre-deployment command was cleared after release.
- The public catalog contains 28 evidence-gated English investor briefs. The new 16-profile Greater Tokyo cluster covers all 23 Tokyo wards through decision-specific submarkets, Tama, Yokohama, Kawasaki, Saitama City, South Saitama, Chiba Bay, and outer-Chiba growth/employment nodes; the hub also includes one Greater Tokyo overview brief.
- Each metro brief shares one DB record across Web, JSON, and Markdown and includes four sourced analysis chapters, 2026 MLIT/REINS evidence, covered-market boundaries, downside risks, decision gates, a 12-item diligence checklist, FAQs, methodology, and five primary/public sources. The live Yokohama page exposes 2,224 readable words and five market evidence points.
- The published UI includes Framer Motion filtering, a Recharts benchmark switch, a browser-local underwriting stress sandbox, responsive evidence cards, search/category/geography controls, and loading/empty/error states. SEO/GEO delivery includes canonical URLs, HTTP 308 locale redirects with query preservation, Article/FAQPage/Dataset/Breadcrumb JSON-LD, dynamic sitemap, `llms.txt`, attributable JSON/Markdown APIs, and curated indexable A/B comparisons; arbitrary pairs remain `noindex`.
- The pSEO manifest models 195,264 candidates, including 5,760 Greater Tokyo strategy/profile/locale combinations. Candidate generation does not publish or index a page: distinct intent, unique sourced analysis, canonical ownership, dynamic evidence, quality scoring, and human translation review remain mandatory gates.
- Production DB read-back passed with `total=28`, `enhanced=16`, temporary helper removed, and anon/authenticated table access denied. Live API, 16 new sitemap URLs, seven new comparison URLs, `llms.txt`, JSON-LD, Markdown depth, curated/noindex policy, and 308 redirects all passed.
- Live Playwright passed the investor/API and Greater Tokyo filter-to-underwriting flow in desktop Chromium and Mobile Safari. Both 1440px and 390px layouts have zero horizontal overflow, zero browser console/page errors, and the full post-deploy release doctor passed, including Cloudflare origin lock, Traefik, Supabase, public routes, and sales health.
- ACTIVE HANDOFF: monitor Search Console index coverage, query clusters, API/x402 usage, and conversion quality. Expand locales and long-tail combinations only after source freshness and human translation review; rankings are an external outcome and are not guaranteed by page count.
