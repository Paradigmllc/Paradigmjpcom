...
- Release blocker: Stripe Dashboard does not expose existing secret values and no Stripe live secret is stored in approved secret references or Coolify. A dedicated Quote Recovery live secret key must be created with action-time approval, then injected with the two Price IDs and webhook signing secret before production Checkout can be truthfully verified.
