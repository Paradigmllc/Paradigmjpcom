...
- ACTIVE HANDOFF: finish the current non-billable QA render, release the GPU, merge/deploy the temporal-fidelity and official-sampler correction, rebind `pet-memory-i2v`, and run a one-shot motion canary before another five-shot film. Do not enable `PET_MOVIE_GPU_RENDER_ENABLED=true` until a real pet-photo MP4 preserves the subject throughout, passes the automated gate, and passes both human reviews. No customer charge or delivery is authorized for QA.
