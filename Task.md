...
- PR #649 / main `22c40b84` and production-wiring hotfix PR #653 / main `879870d9` are merged. Canonical Linux releases `30724102266` and `30724846256` passed routing, Coolify deployment, public funnel, and embedded Video Factory verification.
...
- Release blocker: Price IDs and the webhook signing secret are in Coolify, but a dedicated Stripe live secret key and a Resend account/API key plus `paradigmjp.com` sender-domain connection require action-time approval. After both are injected, production Checkout/Portal/Webhook/password-reset must pass before merge; Linux validation run `30734493770` already passed the production build.
