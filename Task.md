...
- 恒久対策追加: deploy 前フックが Hetzner CPU を Keychain 経由で確認し、過負荷時は deploy を止める。ホストガード script は 5分間隔で disk/load/memory/Coolify/Traefik を記録し、stale Coolify helper を清掃する実装に更新済み。ただし本番ホストへの cron 設置はローカル SSH 鍵（`~/.ssh/id_ed25519` / `~/.ssh/coolify_key`）不在で未実行。大量リストの batch 作成はインライン解析・即時 Twenty 逐次同期を外し、既存 enrichment queue に寄せて HTTP リクエストを長時間占有しない。
