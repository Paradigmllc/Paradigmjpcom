...
- Production QA on the existing managed RTX 3090 generated all five real image-to-video candidates at 576x1024/73 frames. Visual inspection found that the partial-denoise workflow could preserve the opening frame but lose the subject later, so all five shots safely fell back to the supplied-photo edit and checkout remains disabled. The fidelity gate now samples the entire clip instead of only its opening frame, and the workflow is being returned to the official Wan 2.2 20-step/CFG 5/full-denoise sampler contract while retaining the lower-latency native intermediate.
