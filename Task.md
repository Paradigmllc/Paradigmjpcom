...
# CURRENT STATUS - 2026-09-07 editorial QA and GPU stop confirmation

- User requests uninterrupted implementation of the self-owned OSS studio, not more SaaS subscriptions. Implementation committed/pushed as `03f407d0` on draft PR #740 in `codex/video-studio-cost-quality`; current editorial changes are not deployed.
- Added authored chapters/shot timing up to a structural 30-minute limit, explicit per-shot narration/source inputs, Japanese chapter form validation, retained DB JSON payloads, narration preflight before GPU acquisition, and strict partial-render receipts. Actual long-form production quality is NOT established by the schema limit.
- Real local 36-second / 3-chapter Japanese narration draft rendered at 1280x720 / 24fps with HyperFrames 0.7.87, Kokoro ONNX 0.6.1 + Misaki 0.9.4 / pyopenjtalk sandbox audio, and FFmpeg. Technical QA passes; no human approval/delivery. Artifact: `/Users/apple/Desktop/Paradigm-video-QA-20260907/editorial-workspace/projects/editorial-ja-36s-internal-qa/master/master.mp4`. The default espeak Japanese path produced an implausible 23.68-second short sentence; Misaki produced per-shot 2.688-4.416-second clips. This is not a new production voice approval.
- PAID SANDBOX ENDED: Vast **50131323**, label `paradigm-motion-candidate-20260907`, never left loading within the 20-minute bootstrap cutoff. No generation was submitted and 0 MP4 files exist. Harness stopped/destroyed it after 1,216 seconds; `cleanup.json` records removed=true and an independent provider query returned no matching instance. No automatic replacement rental. Production workflow/model approvals unchanged. Exact finalized charge is unverified; do not equate elapsed time with GPU compute billing.
- Fixed a separate live-code defect discovered from that failure: lifecycle cleanup previously treated every non-running state as stopped. It now requires stopped/exited + intended stopped + hardware state stopped, keeps waiting for transitional states, and records/notifies stop_failed when confirmation is absent. Required OSS-worker profile verification now actually prevents readiness after a profile assertion fails. Existing API/GUI/operator-event routes are retained, not a new unattached control plane.
- Release still blocked: current npm audit reports image-size <=2.0.2 affected and npm latest is 2.0.2; Payload latest 3.88.0 also affected. No dependency bypass or production deployment. Local disk has only ~6 GiB available; do not duplicate the root node_modules installation or download large models locally.
- Final verification: full service pytest **150/150**, Ruff, mypy (64 files), root tsc, events Vitest **5/5**, desktop/mobile real planning API E2E **2/2** all pass. Existing Prefect shutdown logging warning persists after successful tests (exit 0). Remote PR head `03f407d0` confirmed; new CI checks were not yet registered at read-back, so no new CI success claimed. Local QA server and dedicated QA browser session stopped. Do not merge, deploy or claim high-quality mass production until the remaining release, provider dispatch/callback accounting and creative review gates are satisfied. Evidence: [editorial QA](docs/knowledge/video-studio-editorial-qa-20260907.md).
...
- Production QA on the existing managed RTX 3090 generated all five real image-to-video candidates at 576x1024/73 frames. Visual inspection found that the partial-denoise workflow could preserve the opening frame but lose the subject later, so all five shots safely fell back to the supplied-photo edit and checkout remains disabled. The fidelity gate now samples the entire clip instead of only its opening frame, and the workflow is being returned to the official Wan 2.2 20-step/CFG 5/full-denoise sampler contract while retaining the lower-latency native intermediate. GPU lifecycle locking is now asynchronous/non-blocking so simultaneous startup and operator reconciliation cannot stall the API event loop.
- The corrected official-sampler one-shot canary generated a real 576x1024/73-frame pet motion clip: the dog and cat remain present at 0/25/50/75/100%, with restrained head, gaze and breathing motion instead of the prior brown-frame collapse. PR #732 deployed the motion-aware structure/entropy gate through production run `30783410618`; the interrupted non-billable run was reconciled safely and managed Vast instance `46258780` is stopped with no active runs or GPU leases.
- Production read-back exposed a second gate defect: comparing the original portrait source and vertical output through independent square crops produced a false 0.500955 rejection. The gate now compares the exact generated aspect-ratio crop while preserving the unchanged 0.78 floor; the original production source plus exact generated MP4 score 0.927432 locally. A regression test covers wide-source-to-vertical output and the flat-collapse rejection remains intact.
- Paid-film pacing now fails closed instead of stretching five photos across 30/60 seconds. Mini requires at least 10 distinct files for 30 seconds; Story/Cinema require 18 for 60 seconds, keeping every shot at roughly 3.33 seconds or less. Every supplied photo is used once, byte-identical uploads and repeated storyboard assets are rejected, captions occupy factual emotional beats rather than every shot, and the plan UI exposes the minimum/ideal photo count before checkout.
- ACTIVE HANDOFF: merge/deploy the aspect-correct fidelity and paid editorial-density release, then run a complete 10-photo/30-second or 18-photo/60-second canary using genuinely distinct source photos and verify every selected GPU shot, the full HyperFrames master, both human reviews, and rights-cleared emotional sound design. Do not enable `PET_MOVIE_GPU_RENDER_ENABLED=true` before that evidence. The current Wan 2.2 TI2V 5B/RTX 3090 canary is improved but is not represented as maximum-spec, superior to a viral reference, or guaranteed to go viral. No customer charge or delivery is authorized for QA.

## CURRENT STATUS - 2026-08-03 Greater Tokyo investor pSEO scenario atlas

- Production release is complete through PRs #725, #726, #728 and #729. Exact main `ad4ece6f` deployed successfully through GitHub run `30778894674` / Coolify deployment `s93woopxfhiflydh6xq1lfus`; the application and embedded runtime verification passed.
- Container startup applied and read back the service-role-only `investor_metro_scenarios` catalog before serving traffic. Production logs confirm 320 indexable scenarios across 16 Greater Tokyo markets and 2,560/2,560 unique long-form paragraphs; the migration also enforces 1,280/1,280 unique section titles, minimum 500-character paragraphs, valid source references, RLS/FORCE RLS and denied anon/authenticated access.
- The live atlas now publishes 16 market hubs and 320 distinct market × locally suitable strategy × investor-profile decision pages. Each scenario exposes four sourced analysis sections, eight substantial paragraphs, editable base/downside underwriting, market evidence, risks, decision gates, checklist, related paths and a scenario-specific 1200x630 social image.
- JSON and Markdown scenario APIs, unified Content API inclusion, investor factory counts, sitemap, robots and `llms.txt` distribute the same evidence-backed records to search engines, answer engines, agents and paid/API workflows. The live factory reports 28 briefs + 18 curated comparisons + 320 scenarios = 366 indexable investor decision pages while retaining 195,264 candidates behind quality gates.
- Live HTTPS read-back passed for readiness, catalog, detail JSON, Markdown, factory, atlas, market hub, scenario HTML, sitemap, robots and `llms.txt`. Yokohama returns 20/20 gated scenarios; the sample returns quality 96, four sections, eight paragraphs and five sources; sitemap contains 840 URLs including exactly 320 scenario URLs.
- Canonical, Article/Dataset/FAQ JSON-LD and the linked underwriting calculator are present in rendered HTML. Two sampled social images returned distinct 1200x630 PNG hashes. The dedicated production scenario flow passed Chromium and mobile-Safari 2/2; the wider repeated regression run passed 13/14 after the inquiry endpoint correctly rate-limited the repeated invalid request with 429 instead of the test's first-request 400 expectation.
- ACTIVE HANDOFF: implementation and production release are complete for this scope. Monitor Search Console index coverage, crawled/discovered ratios, query clusters, source freshness, API/Markdown consumption and qualified conversion; keep future expansion behind the same distinct-intent, source, depth, uniqueness and human-review gates. Rankings are an observed search outcome, not a release-time guarantee.
...
- Shopify CLI linked the existing `SERICIA BASE Sync` Developer Dashboard app and released configuration version `sericia-base-sync-3`. The app grants `write_products`, `write_inventory`, and `read_locations`; Shopify treats the matching read scopes as implicit, so the production scope verifier was corrected to accept that documented behavior and now has regression coverage.
- PR #712 is merged at main `537b899d`; every GitHub check passed and canonical Coolify deployment `gmj8t4sjprx8blusdsugs7pf` finished healthy. Duplicate API deployments were cancelled while still queued so only the canonical release ran.
- Authenticated production canary `2304af51-7a02-448f-b6fb-85faadc25938` returned HTTP 200 `blocked` with 2/12 gates ready: storefront reachability and Shopify Admin API now pass, while the real catalog count remains 0. The snapshot persisted and DB bell plus Slack notification succeeded. `sericia.com` remains 302 to `/password`; unauthenticated launch audit returns 401.
...
- ACTIVE HANDOFF: obtain BASE Developers utilization approval plus Client ID/Secret, complete the existing OAuth callback, then run the scheduled BASE-to-Shopify DRAFT sync against verified real inventory. Product evidence, payments, checkout, policies, and Instagram/Pinterest Business credentials remain blocked. Commercial launch stays password-protected until all evidence exists.
