        ...
        summaries.append(
            {
                ...
                "vram_total_gb": _bytes_to_gb(
                    item.get("vram_total") or item.get("total_memory")
                ),
                "vram_free_gb": _bytes_to_gb(
                    item.get("vram_free") or item.get("free_memory")
                ),
            }
        )
    return summaries
...
def _registry_status(settings: Settings) -> dict[str, object]:
    ...
    items = {str(item["id"]): item for item in status["items"]}  # type: ignore[index]
    ...
    unready = [
        ...
        if item in items
        and not (items[item]["enabled"] and items[item]["workflow_valid"])
    ]
    return {
        **status,
        "exists": True,
...
def doctor_report(settings: Settings) -> dict[str, object]:
    ...
    if settings.comfyui_base_url:
        try:
            ...
            if response.is_success:
                ...
                totals = [
                    float(item["vram_total_gb"])
                    for item in devices
                    if item.get("vram_total_gb") is not None
                ]
                comfy_status["vram_ready"] = bool(totals) and max(totals) >= settings.comfyui_min_vram_gb
    ...
    return {
        ...
        "frameio": {
            "configured": bool(
                settings.frameio_access_token and settings.frameio_create_file_url
            )
        },
