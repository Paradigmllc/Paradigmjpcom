 ...
 * 4. 新 sub pages:
 *    - 🔧 Setup & Environment (Coolify env / event webhook 筁E
 *    - ❁EFAQ
...
async function createSetupPage() {
  return createSubPage(
    ...
    [
      block.callout("Coolify 環墁E��数 + event webhook + Slack 設定�E完�Eリファレンス、E, "⚙︁E, "blue_background"),
      block.p(""),
      ...
      block.p(""),
      block.h2("⏰ Event Webhooks (event webhook)"),
      block.calloutR(
        [
          T("以丁E6 event webhook めEEvent Webhooks に登録すると、Notion ↁESupabase 双方吁Esync が完�E自動化:"),
        ],
        "🤁E,
        "yellow_background",
      ),
...
async function createSyncFlowPage() {
  return createSubPage(
    ...
    [
      ...
      block.p(""),
      block.h3("Notion ↁESupabase (event webhook)"),
      block.bulletR([T("/api/sales/sync-templates-from-notion", { code: true }), T("  ENotion で斁E��編雁EↁEwebhook one-shot ↁESupabase upsert")]),
      ...
      block.callout("チE��プレ編雁EↁEwebhook one-shot ↁE/report/[slug] 最大 6 刁E��本番反映、E, "🎯", "green_background"),
      block.p(""),
  ...
  const blocks = [
    ...
    block.calloutR(
      [
        ...
        T(" Notion 編雁EↁEwebhook one-shot ↁE本番 /report/[slug] が最大 6 刁E��反映、E),
      ],
      "🔄",
      "default",
    ),
    ...
    setupId
      ? block.calloutR(
          [linkT("🔧 Setup & Environment", `https://www.notion.so/${setupId.replace(/-/g, "")}`), T("  ECoolify env / event webhook / Slack 設定�E完�Eリファレンス")],
          "🔧",
          "gray_background",
        )
      : block.p(""),
