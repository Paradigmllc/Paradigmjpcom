...
function checkStaticReleaseRules() {
  ...
  if (
    ...
    && manualWorkHistoryItem.includes("GPT-5.6文面を作る")
    && manualWorkHistoryItem.includes("自動送信: なし")
    && manualMessageIntelligence.includes("企業別文面は未生成です")
    && manualMessageIntelligence.includes("fastQualification")
    && manualMessageIntelligence.includes("定型文は生成しません")
    && manualMessageIntelligence.includes("高品質編集")
    && !manualMessageIntelligence.includes("generation_error")
    && manualWorkOperatorNotice.includes("fastQualification(item)")
    && manualWorkOperatorNotice.includes("この段階ではDeepSeekも定型文も使用しておらず、送信文はまだ作っていません")
