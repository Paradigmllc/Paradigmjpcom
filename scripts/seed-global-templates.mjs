#!/usr/bin/env node
/**
 * scripts/seed-global-templates.mjs — Sprint 16 グローバル版 56 templates (en) 投入
 *
 * 役割: 8 industries × 7 issues = 56 templates の英語ベース seed.
 *       既存 jp 版と同じ 5-stage frame (despair→hope) を encode・英語で書く.
 *       Supabase + Notion グローバル版 4 DB に同時投入.
 */
...
const ISSUE = {
  ...
  ua_残存: { severity: "critical", bounce: 0, fix: 500, cta: "GA4 + Cookie consent setup" },
...
function generate(industryCode, issueCode) {
  ...
  const headlineMap = {
    ...
    ua_残存: `You haven't been able to see who visits your site (since GA4 migration deadline)`,
    ssl_expired: `Chrome and Safari are showing "dangerous site" warnings on your URL`,
    wp_outdated: `Your ${ind.label} site is running outdated WordPress — known vulnerabilities`,
    ...
    copyright_old: `"Is this business still operating?" — your visitors are asking`,
  }
  ...
  const painMap = {
    ...
    ua_残存: `Analytics broken for 2+ years. Where visitors come from, what they see, where they drop off — completely opaque. Ad spend ROI cannot be measured.`,
    ssl_expired: `https:// is broken. Visitors bounce before reaching the booking form. Browser warnings trigger.`,
    wp_outdated: `Known vulnerabilities in older WordPress versions. ${ind.label} sites handle names, phones, and bookings — liability for breach is significant.`,
  ...
  const fearMap = {
    ...
    ua_残存: `Without data, monthly ad spend is "may or may not be working" black-box money. Privacy regulations also apply (GDPR / Cookie consent).`,
  ...
  const lossMap = {
    speed_critical: `Mobile bounce ${iss.bounce}% × 1,200 monthly visitors × $${ind.avg} avg ticket × 2% CVR = ~$${monthlyLoss.toLocaleString()}/month (~$${annualLoss}K/year) opportunity loss`,
    ua_残存: `Without measurable ROI, $2K/month ad spend assumption = $24K/year of black-box expense. Estimated $${annualLoss || 50}K/year improvement opportunity remains untapped.`,
    ssl_expired: `Warning-driven bounce ${iss.bounce}% × 1,200 visitors = ~1,020/month lost. At $${ind.avg} ticket × 2% CVR → $${annualLoss || 100}K+/year loss.`,
    wp_outdated: `Breach legal cost: $300+ per record × ${ind.label} customer DB of ~1,000 records = $300K liability. Probability ~8%/year for outdated versions.`,
    no_ogp: `Social share rate 5% × 1,200 visitors × 1/4 CTR loss = ~45 new entries/month lost. Annually: 540 entries × 2% CVR × $${ind.avg} = $${Math.round((540 * 0.02 * ind.avg) / 1000)}K/year.`,
  ...
  const ctaMap = {
    speed_critical: `${iss.cta} — Paradigm gets your PageSpeed to 80+ in 14 days. From $${iss.fix}.`,
    ua_残存: `${iss.cta} — GA4 + Cookie consent banner + data restoration. From $${iss.fix}.`,
    ssl_expired: `${iss.cta} — SSL re-issue + server infrastructure review. From $${iss.fix}.`,
    wp_outdated: `${iss.cta} — Emergency update + automated backups + WAF. From $${iss.fix}.`,
    no_ogp: `${iss.cta} — OGP image generator + Twitter Card + LINE share. From $${iss.fix}.`,
    no_sns: `${iss.cta} — Instagram + Google Business Profile integration + review automation. From $${iss.fix}.`,
    copyright_old: `${iss.cta} — Automatic copyright update + site-wide freshness audit. From $${iss.fix}.`,
  }
...
async function main() {
  ...
  console.log(`✅ Supabase: ${data.length} global templates inserted`)
  ...
  console.log(`✅ Notion: ${notionCreated} global templates created`)
  console.log(`\nNext: /api/sales/sync-templates-from-notion で 5min ごとに自動 sync`)
}
