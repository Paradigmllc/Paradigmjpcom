...
export function SalesReportVideoStudioPanel({ data }: { data: SalesDashboardData }) {
  ...
  async function renderJob(jobId: string) {
    ...
    try {
      ...
      toast.success("GPUなしレポート動画を生成しました")
      await refreshJobs()
    } catch (error) {
  ...
  return (
    <div className="grid gap-5 bg-zinc-50 p-5 xl:grid-cols-[minmax(380px,460px)_minmax(0,1fr)]">
      <section className="rounded-2xl border border-zinc-200 bg-white">
        <div className="border-b border-zinc-200 p-5">
          <div className="flex items-start justify-between gap-3">
            ...
            <button type="button" onClick={() => void refreshJobs()} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-200" aria-label="レポート動画を更新">
        ...
        <div className="grid gap-4 p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <label className="grid gap-1.5 text-xs font-medium text-zinc-600">
              <span>尺</span>
              <input value={duration} onChange={(event) => setDuration(event.target.value)} className="h-10 rounded-lg border border-zinc-300 px-3 text-sm text-zinc-950 outline-none focus:border-zinc-900" />
            </label>
          </div>
          <label className="grid gap-1.5 text-xs font-medium text-zinc-600">
            <span>CTA</span>
      ...
      <section className="grid gap-5">
        <div className="grid gap-3 md:grid-cols-3">
          <PipelineCard name="HyperFrames" ready={config.renderers.hyperframes} note="診断レポート解説、数字カード、CTAをGPUなしでレンダーします。" icon={FileVideo} />
          <PipelineCard name="Cloudflare R2" ready={config.r2.ready} note="承認後のMP4、字幕、サムネイル、メタデータを保存します。" icon={UploadCloud} />
          <PipelineCard name="GPU" ready={true} note="このスタジオではVast.ai/ComfyUIを使いません。速さと再現性を優先します。" icon={CheckCircle2} />
        </div>
