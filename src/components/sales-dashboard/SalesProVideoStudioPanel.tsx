...
export function SalesProVideoStudioPanel({ data }: { data: SalesDashboardData }) {
  ...
  async function dispatchJob(jobId: string) {
    ...
    try {
      ...
      const json = (await res.json()) as ActionResponse
      if (!res.ok || !json.ok || !json.job) throw new Error(json.error ?? "n8n投入に失敗しました")
      setJobs((rows) => rows.map((job) => (job.id === jobId ? (json.job as SalesVideoJob) : job)))
      toast.success(json.message ?? "制作ジョブを投入しました")
    } catch (error) {
      console.error("[pro-video-studio] dispatch failed:", error)
      toast.error(error instanceof Error ? error.message : "投入に失敗しました")
    } finally {
      setBusy(null)
    }
  }
  ...
  const studioLinks = [
    ...
    { label: "n8n", url: toolUrl(data, "n8n", "https://n8n.paradigmjp.com") },
    { label: "OpenMontage", url: process.env.NEXT_PUBLIC_OPENMONTAGE_STUDIO_URL?.trim() || "https://github.com/calesthio/OpenMontage" },
  ]

  return (
    <div className="grid gap-5 bg-zinc-50 p-5 xl:grid-cols-[minmax(420px,500px)_minmax(0,1fr)]">
          ...
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <p className="mt-2 text-sm leading-6 text-zinc-600">
                顧客納品・ショート・アバター・B-roll向けのGPUありラインです。GPUはVast.aiだけを使い、ComfyUI APIはヘッドレス実行します。
              </p>
            </div>
            <button type="button" onClick={() => void refreshJobs()} className="inline-flex h-9 w-9 items-center justify-center rounded-lg border border-zinc-200" aria-label="プロ級動画を更新">
      ...
      <section className="grid gap-5">
        <div className="grid gap-3 md:grid-cols-2 xl:grid-cols-4">
          ...
          <PipelineCard name="Cloudflare R2" ready={config.r2.ready} note="入力素材、生成素材、初稿、最終納品物を保存します。" icon={UploadCloud} />
        </div>
        <div className="rounded-2xl border border-zinc-200 bg-white p-5">
          <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
            <div>
              ...
              <p className="mt-1 text-xs text-zinc-500">対象: {selectedCompany?.companyName ?? "未選択"}。細かいworkflow調整はOSS正規GUIで行い、量産はAPIで実行します。</p>
            </div>
