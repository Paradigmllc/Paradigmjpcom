    ...
    <section id="create" className="relative overflow-hidden bg-paradigm-paper-deep py-20 sm:py-28">
      ...
      {preview && (
        <div className="mx-auto mt-24 max-w-6xl px-5 sm:px-8">
          ...
          <div className="grid gap-5 md:grid-cols-3">{PET_MOVIE_PLANS.map((plan) => { const recommended = plan.id === "story"; return <Card key={plan.id} className={`relative overflow-hidden rounded-[1.75rem] bg-paradigm-paper-card ${recommended ? "border-paradigm-accent shadow-[0_20px_60px_rgba(111,70,245,.18)]" : "border-paradigm-line"}`}>{recommended && <div className="bg-paradigm-accent px-4 py-2 text-center text-[10px] font-bold uppercase tracking-[.16em] text-white">{x.recommended}</div>}<CardContent className="p-7"><div className="mb-6 flex items-start justify-between gap-4"><div><p className="font-display text-2xl">{plan.name}</p><p className="mt-1 text-xs text-paradigm-ink-mute">{plan.durationSeconds} sec · {plan.formats.join(" + ")}</p></div><span className="text-3xl font-bold tracking-tight">${plan.priceUsd}</span></div><ul className="mb-7 space-y-3 text-sm text-paradigm-ink-soft"><li className="flex gap-2"><Check className="h-4 w-4 shrink-0 text-emerald-600" aria-hidden="true" />{t.noWatermark}</li><li className="flex gap-2"><Check className="h-4 w-4 shrink-0 text-emerald-600" aria-hidden="true" />{t.identitySafe}</li><li className="flex gap-2"><Check className="h-4 w-4 shrink-0 text-emerald-600" aria-hidden="true" />{x.oneTime}</li><li className="flex gap-2"><Check className="h-4 w-4 shrink-0 text-emerald-600" aria-hidden="true" />{x.delivery}</li></ul><Button className={`h-12 w-full rounded-full ${recommended ? "bg-paradigm-ink text-paradigm-paper hover:bg-paradigm-accent" : ""}`} variant={recommended ? "default" : "outline"} disabled={busy || !checkoutEnabled} onClick={() => startCheckout(plan.id)}>{checkoutEnabled ? t.choosePlan.replace("{plan}", plan.name) : t.paidSoon}</Button></CardContent></Card> })}</div>
        </div>
      )}
    </section>
  )
