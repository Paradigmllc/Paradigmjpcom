...
function QualityForm({ dashboard, busy, onSave }: { dashboard: GenerationControlDashboard; busy: boolean; onSave: (body: Record<string, unknown>) => Promise<void> }) {
  ...
  return <Card><CardHeader><CardTitle className="text-base">6軸品質比較</CardTitle></CardHeader><CardContent className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
    ...
    <label className="flex items-center gap-2 text-sm font-semibold"><input type="checkbox" checked={approved} onChange={(event) => setApproved(event.target.checked)} />再利用を許可</label>
...
export function VideoStudioControlDashboard() {
  ...
  return <main className="min-h-dvh bg-zinc-50 px-4 py-6 sm:px-6 sm:py-10"><div className="mx-auto max-w-[1500px] space-y-6">
    <header className="rounded-2xl border border-zinc-200 bg-white p-5 shadow-sm sm:p-7"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[0.2em] text-blue-700">Video Studio Control Plane</p><h1 className="mt-2 text-2xl font-black sm:text-3xl">品質・費用・Vast.ai安定運用</h1><p className="mt-3 max-w-3xl text-sm leading-6 text-zinc-600">課金前に日次/実行/token/GPU秒を原子的に検査し、重複は再利用、連続失敗はprovider単位で遮断します。この画面のpreflightはGPUも有料APIも起動しません。</p></div><div className="flex gap-2"><Button variant="outline" onClick={() => void refresh()} disabled={busy}><RefreshCw />更新</Button><Button asChild variant="outline"><Link href="/video-factory-console" prefetch={false}>Factory Console</Link></Button></div></div></header>
