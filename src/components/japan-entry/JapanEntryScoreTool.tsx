...
const COPY: Record<LocaleVariant, {
  ...
  en: {
    ...
    apply: "Apply for a 21-day Japan Entry plan — $15K",
  ...
  ja: {
    ...
    apply: "14営業日のJapan Entry計画に申込む — $15K",
    retry: "もう一度診断する",
    bands: {
      "no-data": "有効なシグナルなし",
        ...
        <section id="japan-entry-score-result" tabIndex={-1} className="border-t border-zinc-200 bg-zinc-50 px-5 py-16 outline-none sm:px-8 sm:py-20 lg:px-12">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-8 flex flex-col items-start justify-between gap-5 rounded-2xl bg-zinc-950 p-6 text-white sm:flex-row sm:items-center sm:p-8">
              <div><p className="text-lg font-semibold">{copy.apply}</p><p className="mt-2 text-sm text-zinc-300">{locale === "ja" ? "セットアップ15,000ドル固定。選定した契約先には月額2,000ドル×3か月＝6,000ドル相当の運用を追加月額なしで提供し、期間終了後の継続条件・月額は個別協議のうえ書面で合意します。" : "Fixed $15,000 setup. Selected launch partners receive $2,000/month × 3 months = $6,000 of standard managed-operation value included at no additional monthly fee; month 4 onward is $2,000/month under the signed terms."}</p></div>
