...
export function ManualMessageIntelligence({ item, onCopy }: {
      ...
      <section className="overflow-hidden rounded-xl border border-amber-200 bg-amber-50" aria-label="フォーム文面の生成状況">
        ...
        <div className="border-t border-amber-200 px-4 py-3 text-xs leading-5 text-amber-900">
          ...
          <p className="mt-1 font-semibold">{fastQualification
            ? "低優先候補のため、高速判定では文面を作成していません。"
            : "初回処理内の自動生成・品質修正・再生成は完了しています。"}</p>
        </div>
      </section>
    )
  }
  const generationStatus = text(review.generation_status) ?? ""
  const deterministic = generationStatus.startsWith("deterministic_")
  ...
  const usage = deterministic ? null : trackedDeepSeekUsage(item)
  const productLabel = text(review.product_label)
  const customerPathGap = text(review.customer_path_gap)
  ...
  return (
    <details className="overflow-hidden rounded-xl border border-slate-200 bg-white" open>
      <summary className="flex cursor-pointer list-none items-center gap-2 px-4 py-3 text-sm font-semibold text-slate-800 marker:hidden">
        <MessageSquareText className="size-4 text-blue-600" />{deterministic ? "短文初回メッセージ（未送信）" : "企業別フォーム文面（未送信）"}
        <span className="ml-auto flex gap-1.5">
          {score !== null && <Badge variant="outline" className="border-emerald-200 bg-emerald-50 text-emerald-700">品質 {score}</Badge>}
          {!deterministic && personalizationScore !== null && <Badge variant="outline" className="border-fuchsia-200 bg-fuchsia-50 text-fuchsia-700">個別性 {personalizationScore}</Badge>}
          {!deterministic && uniqueness !== null && <Badge variant="outline" className="border-blue-200 bg-blue-50 text-blue-700">固有性 {uniqueness}</Badge>}
      ...
      <div className="space-y-4 border-t border-slate-100 bg-slate-50/60 p-4">
        <div className="rounded-xl border border-blue-200 bg-white p-4">
          <div className="flex items-center gap-2 text-xs font-semibold text-blue-800"><CheckCircle2 className="size-4" />{deterministic ? "短文・事実限定で生成" : "DeepSeek批評で採用"}</div>
        ...
        {deterministic && (productLabel || customerPathGap) && <div className="grid gap-2 sm:grid-cols-2"><div className="rounded-lg border border-slate-200 bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Product anchor</p><p className="mt-1 text-xs leading-5 text-slate-700">{productLabel ?? "公開商品ページ"}</p></div><div className="rounded-lg border border-slate-200 bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Japan gap</p><p className="mt-1 text-xs leading-5 text-slate-700">{customerPathGap ?? "日本向け導線"}</p></div></div>}
        {!deterministic && personalization && <div className="rounded-xl border border-fuchsia-200 bg-fuchsia-50/60 p-4" aria-label="企業別文面品質ゲート"><div className="flex flex-wrap items-center gap-2"><p className="text-xs font-semibold text-fuchsia-950">テンプレ感・営業品質ゲート</p>{architecture && <Badge variant="outline" className="border-fuchsia-200 bg-white text-fuchsia-800">構成 {architecture}</Badge>}</div><div className="mt-3 grid gap-2 sm:grid-cols-2 lg:grid-cols-4">{[["企業固有性", "companySpecificity"], ["構成独自性", "narrativeOriginality"], ["営業関連性", "commercialRelevance"], ["言語品質", "languageIntegrity"]].map(([label, key]) => <div key={key} className="rounded-lg border border-fuchsia-100 bg-white p-3"><p className="text-[10px] font-bold uppercase tracking-wider text-fuchsia-700">{label}</p><p className="mt-1 text-sm font-semibold text-slate-900">{number(personalizationDimensions?.[key]) ?? "-"} / 25</p></div>)}</div><p className="mt-3 text-[11px] leading-5 text-fuchsia-900">企業名・商品根拠・日本向け判断・解決焦点・固有CTA・過去文面との差分・異言語/UI文言混入を保存前に検査しています。</p></div>}
