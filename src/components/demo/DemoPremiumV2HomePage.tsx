...
export function DemoPremiumV2HomePage({ data }: { data: DemoMultiPageData }) {
  ...
  return (
    <div className="overflow-hidden bg-[#f4f1e9] text-[#171713]" style={{ "--v2-accent": data.meta.accentColor } as React.CSSProperties}>
      <section className="relative min-h-[820px] lg:min-h-[calc(100svh-7rem)]">
        ...
        <div className="relative mx-auto flex min-h-[820px] max-w-[1500px] flex-col justify-end px-5 pb-14 pt-28 text-white sm:px-10 sm:pb-20 lg:min-h-[calc(100svh-7rem)] lg:px-16 lg:pb-24">
          <Reveal>
            <div className="mb-8 flex items-center gap-3 text-xs font-bold uppercase tracking-[.24em] text-white/75"><BadgeCheck className="h-4 w-4" />Verified-source private preview</div>
            <h1 className="max-w-5xl whitespace-pre-line font-premium-serif text-[clamp(3.5rem,8.3vw,8.4rem)] leading-[.88] tracking-[-.065em] text-balance">{home.hero.title}</h1>
      ...
      <section className="grid border-y border-black/10 lg:grid-cols-[.42fr_1.58fr]">
        <div className="flex min-h-40 items-center bg-[#171713] px-6 py-10 text-white sm:px-12 lg:min-h-[500px] lg:items-end">
          <p className="max-w-xs text-xs font-semibold uppercase leading-6 tracking-[.28em] text-white/55">Actual identity<br />Editorial direction<br />Responsive system</p>
        </div>
        <div className="px-5 py-20 sm:px-10 sm:py-28 lg:px-20 lg:py-36">
          <Reveal className="mx-auto max-w-5xl">
            ...
            <div className="mt-10 grid gap-8 md:grid-cols-2 md:gap-16"><p className="whitespace-pre-line text-base leading-8 text-black/68 sm:text-lg sm:leading-9">{premium.intro.body}</p><div className="border-l border-black/15 pl-7"><p className="text-sm leading-7 text-black/55">{premium.intro.note ?? "事実情報と承認済み素材を基に、ブランド固有の編集設計へ落とし込んでいます。"}</p><a href={`${basePath}/services`} className="group mt-8 inline-flex items-center gap-4 text-sm font-bold">詳しく見る<span className="grid h-11 w-11 place-items-center rounded-full bg-black text-white transition group-hover:translate-x-1"><ArrowRight className="h-4 w-4" /></span></a></div></div>
          </Reveal>
        </div>
      </section>
      ...
      <section className="bg-[#d9d2c2] px-5 py-20 sm:px-10 sm:py-28 lg:px-16 lg:py-36">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-14 flex flex-col justify-between gap-7 border-b border-black/20 pb-9 md:flex-row md:items-end"><div><p className="text-xs font-bold uppercase tracking-[.3em] text-black/50">Signature experiences</p><h2 className="mt-5 font-premium-serif text-4xl tracking-[-.045em] sm:text-6xl">らしさを、ページの隅々まで。</h2></div><p className="max-w-sm text-sm leading-7 text-black/55">同じテンプレートの差し替えではなく、素材・業態・語り口に合わせて構成と余白を変えています。</p></Reveal>
      ...
      <section className="grid bg-[#161612] text-white lg:grid-cols-2">
        ...
        <div className="flex items-center px-5 py-20 sm:px-10 sm:py-28 lg:px-20">
          <Reveal><p className="text-xs font-bold uppercase tracking-[.3em] text-white/45">A complete digital presence</p><h2 className="mt-7 font-premium-serif text-4xl leading-tight tracking-[-.04em] sm:text-6xl">写真が変われば、<br />体験まで変わる。</h2><p className="mt-8 max-w-xl text-base leading-8 text-white/60">ロゴや実際の店舗・商品写真を中心に据えることで、訪問者が一目で「この会社のサイトだ」と理解できる設計です。</p><div className="mt-12 space-y-0 border-y border-white/15">{["全固定ページを同じブランド設計で統一", "スマートフォンで写真とCTAを最適化", "素材の出所と利用根拠を審査記録"].map((item, index) => <div key={item} className="flex gap-5 border-b border-white/15 py-5 last:border-0"><span className="text-white/30">0{index + 1}</span><p className="font-semibold">{item}</p></div>)}</div></Reveal>
        </div>
      </section>

      <section className="px-5 py-20 sm:px-10 sm:py-28 lg:px-16 lg:py-36">
        <div className="mx-auto max-w-7xl"><Reveal className="mb-10"><p className="text-xs font-bold uppercase tracking-[.3em] text-[var(--v2-accent)]">Selected scenes</p><h2 className="mt-5 font-premium-serif text-4xl tracking-[-.045em] sm:text-6xl">ブランドをつくる、実際の景色。</h2></Reveal><div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-12">{premium.gallery.slice(0, 5).map((media, index) => <Reveal key={`${media.src}-${index}`} delay={index * .05} className={index === 0 ? "lg:col-span-7 lg:row-span-2" : "lg:col-span-5"}><figure className={`group relative overflow-hidden bg-black/10 ${index === 0 ? "aspect-[4/5] lg:h-full" : "aspect-[16/10]"}`}><PremiumMedia media={media} className="absolute inset-0" /><div className="absolute inset-0 bg-gradient-to-t from-black/55 via-transparent to-transparent" /><figcaption className="absolute bottom-5 left-5 right-5 text-sm font-semibold text-white">{media.caption}</figcaption></figure></Reveal>)}</div></div>
      </section>
...
function PremiumMedia({ media, priority = false, className = "" }: { media: DemoPremiumMedia; priority?: boolean; className?: string }) {
  if (media.kind === "video") return <video src={media.src} autoPlay muted loop playsInline preload="metadata" aria-label={media.alt} className={`${className} h-full w-full object-cover`} />
  return <div className={`${className} overflow-hidden`}><Image src={media.src} alt={media.alt} fill priority={priority} sizes="(max-width: 1024px) 100vw, 60vw" className="object-cover transition-transform duration-[1600ms] ease-out group-hover:scale-[1.035]" /></div>
}

function Reveal({ children, delay = 0, className = "" }: { children: React.ReactNode; delay?: number; className?: string }) {
  const reducedMotion = useReducedMotion()
  return <motion.div className={className} initial={reducedMotion ? false : { opacity: 0, y: 26 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-70px" }} transition={{ duration: .72, delay, ease: [0.22, 1, 0.36, 1] }}>{children}</motion.div>
}
