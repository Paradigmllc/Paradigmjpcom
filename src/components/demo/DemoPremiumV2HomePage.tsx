...
export function DemoPremiumV2HomePage({ data }: { data: DemoMultiPageData }) {
  ...
  const heroAlign = (data.designRecipe?.compositionVariant ?? 0) % 3 === 1 ? "lg:items-center lg:text-center" : ""
  ...
  return (
    <div className="overflow-hidden bg-[#f4f1e9] text-[#171713]" style={{ "--v2-accent": data.meta.accentColor } as React.CSSProperties}>
      <section className="relative min-h-[720px] lg:min-h-[calc(100svh-5.5rem)]">
        ...
        <div className={`relative mx-auto flex min-h-[720px] max-w-[1500px] flex-col justify-end px-5 pb-14 pt-28 text-white sm:px-10 sm:pb-20 lg:min-h-[calc(100svh-5.5rem)] lg:px-16 lg:pb-24 ${heroAlign}`}>
          <Reveal>
      ...
      <section className="bg-[#d9d2c2] px-5 py-20 sm:px-10 sm:py-28 lg:px-16 lg:py-36">
        <div className="mx-auto max-w-7xl">
          <Reveal className="mb-14 flex flex-col justify-between gap-7 border-b border-black/20 pb-9 md:flex-row md:items-end"><div><p className="text-xs font-bold uppercase tracking-[.3em] text-black/50">{presentation?.featureEyebrow ?? home.featureEyebrow ?? "HIGHLIGHTS"}</p><h2 className="mt-5 whitespace-pre-line font-premium-serif text-4xl tracking-[-.045em] sm:text-6xl">{presentation?.featureHeading ?? home.featureHeading ?? "大切にしていること。"}</h2></div><p className="max-w-sm text-sm leading-7 text-black/55">{home.featureSubtitle ?? data.pages.services.subtitle}</p></Reveal>
