  ...
  return (
    <div className="overflow-hidden bg-[#f4f1e9] text-[#171713]">
              ...
              return (
                <PremiumV2Reveal key={service.title} className={`grid gap-8 lg:grid-cols-2 lg:items-stretch ${index % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""}`}>
                  ...
                  <div className="flex flex-col justify-center border-y border-black/15 py-9 lg:px-12">
                    <p className="text-[10px] font-bold uppercase tracking-[.28em] text-[var(--demo-accent)]">{service.priceNote}</p>
      ...
      <section className="bg-[#171713] px-5 py-20 text-white sm:px-10 sm:py-28 lg:px-16 lg:py-36">
        <div className="mx-auto max-w-7xl">
          <PremiumV2Reveal className="mb-12"><p className="text-xs font-bold uppercase tracking-[.3em] text-white/40">{services.processEyebrow}</p><h2 className="mt-5 font-premium-serif text-4xl tracking-[-.045em] sm:text-6xl">{services.processTitle}</h2></PremiumV2Reveal>
      ...
      <section className="bg-[var(--demo-accent)] px-5 py-20 text-white sm:px-10 sm:py-24 lg:px-16">
        <PremiumV2Reveal className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[1fr_auto] lg:items-end">
          ...
          <a href={services.ctaHref} target="_blank" rel="noopener noreferrer" className="inline-flex min-h-13 items-center justify-center gap-3 bg-white px-8 text-sm font-bold text-black">{services.ctaText}<ArrowUpRight className="h-4 w-4" /></a>
        </PremiumV2Reveal>
      </section>
    </div>
  )
