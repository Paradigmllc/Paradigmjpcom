        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="grid gap-px bg-[var(--demo-line)] md:grid-cols-2">
            {services.services.map((service, index) => {
              ...
              return <PremiumV3Reveal key={service.title} motionStyle={motionStyle} delay={index * 0.04} className="group bg-[var(--demo-surface)]"><div className="relative aspect-[16/10] overflow-hidden"><PremiumV3Media media={itemMedia} className="absolute inset-0" sizes="(max-width:768px) 100vw, 50vw" /><div className="absolute inset-0 bg-gradient-to-t from-black/46 via-transparent to-transparent" /><span className="absolute bottom-5 left-5 text-xs font-bold tracking-[.24em] text-white/78">MENU {String(index + 1).padStart(2, "0")}</span></div><div className="p-7 sm:p-9"><h3 className={`${demoHeadlineClass(service.title, "card")} font-[var(--demo-heading-weight)] [font-family:var(--demo-font-display)]`}>{service.title}</h3><p className="mt-5 text-sm leading-8 text-[var(--demo-muted)]">{service.description}</p><ul className="mt-7 grid gap-3 border-t border-[var(--demo-line)] pt-5 sm:grid-cols-2">{service.features.map((feature) => <li key={feature} className="flex items-start gap-3 text-xs leading-6"><Check className="mt-1 h-3.5 w-3.5 shrink-0 text-[var(--demo-accent)]" />{feature}</li>)}</ul></div></PremiumV3Reveal>
            })}
          </div>
        </div>
      </section>
