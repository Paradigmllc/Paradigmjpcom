...
export function PremiumV3MediaCarousel({ media, label }: { media: DemoPremiumMedia[]; label: string }) {
        ...
        <div className="flex touch-pan-y">
          {media.map((item, index) => {
            ...
            return (
              <motion.div
                key={`${item.src}-${index}`}
                className="min-w-0 shrink-0 basis-[88%] px-2 sm:basis-[68%] lg:basis-[58%]"
                animate={reducedMotion ? undefined : { scale: selected ? 1 : 0.92, opacity: selected ? 1 : 0.48 }}
                transition={{ duration: 0.75, ease: [0.22, 1, 0.36, 1] }}
              >
                <figure className="group relative aspect-[4/3] cursor-grab overflow-hidden bg-black/10 active:cursor-grabbing sm:aspect-[16/10]">
                  <PremiumV3Media media={item} className="absolute inset-0" sizes="(max-width: 640px) 88vw, 62vw" />
                  ...
                  <figcaption className="absolute inset-x-0 bottom-0 flex items-end justify-between gap-4 p-5 text-white sm:p-8">
                    <span className="max-w-lg text-xl leading-tight sm:text-3xl [font-family:var(--demo-font-display)]">{item.caption ?? item.alt}</span>
