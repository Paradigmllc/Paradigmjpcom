...
function pageHero(data: DemoMultiPageData, page: DemoContentPage, media: ReturnType<typeof pageMedia>[number]) {
  ...
  return <PremiumV3PageHero title={page.title} subtitle={page.subtitle} eyebrow={page.eyebrow} media={media} mediaGallery={direction.hero === "mosaic" ? pageMedia(data) : undefined} recipe={data.designRecipe} variant={direction.hero} />
}
...
function WorksPage({ data, page }: { data: DemoMultiPageData; page: DemoContentPage }) {
  ...
  return (
    <div className="overflow-hidden bg-[var(--demo-surface)] text-[var(--demo-ink)]">
      ...
      <section className="px-5 py-20 sm:px-10 sm:py-28 lg:px-16"><div className="mx-auto max-w-7xl"><PremiumV3Reveal motionStyle={motionStyle} className="grid gap-8 border-b border-[var(--demo-line)] pb-10 lg:grid-cols-[1.1fr_.9fr]"><h2 className="whitespace-pre-line text-4xl leading-[1.12] tracking-[-.035em] sm:text-6xl [font-family:var(--demo-font-display)]">{data.presentation?.worksLead ?? `${isRestaurant ? "写真から伝わる、" : "実績から見える、"}\n${data.companyName}${isRestaurant ? "の空気。" : "の仕事。"}`}</h2><p className="self-end text-base leading-9 text-[var(--demo-muted)]">{data.presentation?.worksDescription ?? (isRestaurant ? "料理、空間、接客。その場所で過ごす時間を、確認できる写真と言葉を組み合わせてご紹介します。" : "提供するサービスだけでなく、取り組み方や仕事の細部も大切な判断材料です。確認できる事実とビジュアルを組み合わせてご紹介します。")}</p></PremiumV3Reveal></div></section>
      {direction.worksLayout === "case-grid" ? <section className="px-5 pb-20 sm:px-10 sm:pb-28 lg:px-16 lg:pb-32"><div className="mx-auto grid max-w-[1500px] gap-px bg-[var(--demo-line)] md:grid-cols-2 lg:grid-cols-3">{page.sections.map((section, index) => { const itemMedia = media[index % media.length] ?? hero; return <PremiumV3Reveal key={section.id} motionStyle={motionStyle} delay={index * 0.05} className="group bg-[var(--demo-surface)]"><div className="relative aspect-[4/3] overflow-hidden"><PremiumV3Media media={itemMedia} className="absolute inset-0" sizes="(max-width:768px) 100vw, 34vw" /><span className="absolute left-4 top-4 bg-[var(--demo-ink)] px-3 py-2 text-[10px] font-bold tracking-[.25em] text-white">{isRestaurant ? "SCENE" : "CASE"} {String(index + 1).padStart(2, "0")}</span></div><div className="p-7 sm:p-8"><h2 className="text-3xl tracking-[-.03em] [font-family:var(--demo-font-display)]">{section.heading}</h2><p className="mt-5 whitespace-pre-line text-sm leading-8 text-[var(--demo-muted)]">{customerFacingSceneBody(section.body)}</p></div></PremiumV3Reveal>})}</div></section> : <section className="pb-20 sm:pb-28 lg:pb-32"><div className="mx-auto max-w-[1500px] space-y-20 px-5 sm:px-10 lg:space-y-28 lg:px-16">{page.sections.map((section, index) => { const itemMedia = media[index % media.length] ?? hero; return <PremiumV3Reveal key={section.id} motionStyle={motionStyle} className={`grid gap-8 lg:grid-cols-[1.2fr_.8fr] lg:items-center ${index % 2 === 1 ? "lg:[&>*:first-child]:order-2" : ""}`}><div className="group relative min-h-[420px] overflow-hidden sm:min-h-[560px]"><PremiumV3Media media={itemMedia} className="absolute inset-0" sizes="(max-width:1024px) 100vw, 60vw" /><span className="absolute left-5 top-5 bg-black/72 px-3 py-2 text-[10px] font-bold tracking-[.25em] text-white">0{index + 1}</span></div><div className="lg:px-10"><p className="text-xs font-bold uppercase tracking-[.3em] text-[var(--demo-accent)]">{isRestaurant ? "Scene" : "Scene"} {String(index + 1).padStart(2, "0")}</p><h2 className="mt-5 text-4xl tracking-[-.03em] sm:text-5xl [font-family:var(--demo-font-display)]">{section.heading}</h2><p className="mt-7 whitespace-pre-line text-base leading-9 text-[var(--demo-muted)]">{customerFacingSceneBody(section.body)}</p></div></PremiumV3Reveal>})}</div></section>}
...
function customerFacingSceneBody(body: string): string {
  const cleaned = body.replace(/(?:生成イメージ|AI生成|エキテン掲載素材|権利確認前|提案用素材)[。\s]*/gu, "").trim()
  return cleaned || "店内や料理の雰囲気をご紹介します。"
}
...
function BeautyWorksPage({ data, page }: { data: DemoMultiPageData; page: DemoContentPage }) {
  ...
  return (
    <div className="overflow-hidden bg-[var(--demo-surface)] text-[var(--demo-ink)] [font-feature-settings:'palt']">
      <PremiumV3PageHero title={page.title} subtitle={page.subtitle} eyebrow="スタイル・サロンの風景" media={hero} mediaGallery={media} recipe={data.designRecipe} variant={direction.hero} mosaicLayout="lookbook" />
      ...
      <section className="px-5 pb-20 sm:px-10 sm:pb-28 lg:px-16 lg:pb-36"><div className="mx-auto grid max-w-[1500px] gap-5 lg:grid-cols-2 lg:gap-8">{entries.map((section, index) => {
        const itemMedia = media[index % media.length] ?? hero
        const featured = index % 3 === 0
        return <PremiumV3Reveal key={section.id} motionStyle={motionStyle} delay={index * 0.04} className={`group border border-[var(--demo-line)] bg-[var(--demo-surface)] ${featured ? "lg:col-span-2 lg:grid lg:grid-cols-[1.08fr_.92fr]" : ""}`}><div className={`relative overflow-hidden bg-[var(--demo-surface-alt)] ${featured ? "min-h-[360px] sm:min-h-[520px]" : "aspect-[4/3]"}`}><PremiumV3Media media={itemMedia} className="absolute inset-0" sizes={featured ? "(max-width:1024px) 100vw, 54vw" : "(max-width:1024px) 100vw, 50vw"} /><div className="absolute inset-0 ring-1 ring-inset ring-black/5" /><span className="absolute left-5 top-5 bg-white/92 px-4 py-2 text-[10px] font-bold tracking-[.22em] text-black">LOOK {String(index + 1).padStart(2, "0")}</span></div><div className={`flex flex-col justify-between p-7 sm:p-10 ${featured ? "lg:p-14" : "min-h-72"}`}><div><p className="text-[10px] font-bold tracking-[.24em] text-[var(--demo-accent)]">SALON JOURNAL</p><h2 className={`${demoHeadlineClass(section.heading, featured ? "section" : "card")} mt-5 font-[var(--demo-heading-weight)] [font-family:var(--demo-font-display)]`}>{section.heading}</h2><p className="mt-6 whitespace-pre-line text-sm leading-8 text-[var(--demo-muted)] sm:text-base sm:leading-9">{section.body}</p></div><p className="mt-9 border-t border-[var(--demo-line)] pt-5 text-[10px] tracking-[.2em] text-[var(--demo-muted)]">{data.companyName} / {String(index + 1).padStart(2, "0")}</p></div></PremiumV3Reveal>
      })}</div></section>
...
function NewsPage({ data, page }: { data: DemoMultiPageData; page: DemoContentPage }) {
  ...
  return (
    <div className="overflow-hidden bg-[var(--demo-surface)] text-[var(--demo-ink)]">
      {pageHero(data, page, hero)}
      <PremiumV3Journal data={data} mode="page" />
