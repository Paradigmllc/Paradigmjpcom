...
function BeautyWorksPage({ data, page }: { data: DemoMultiPageData; page: DemoContentPage }) {
  ...
  return (
    <div className="overflow-hidden bg-[var(--demo-surface)] text-[var(--demo-ink)] [font-feature-settings:'palt']">
      ...
      <section className="px-5 pb-20 sm:px-10 sm:pb-28 lg:px-16 lg:pb-36"><div className="mx-auto grid max-w-[1500px] gap-5 sm:grid-cols-2 lg:grid-cols-12">{entries.map((section, index) => {
        const itemMedia = media[index % media.length] ?? hero
        const span = index % 3 === 0 ? "lg:col-span-7" : index % 3 === 1 ? "lg:col-span-5 lg:pt-24" : "lg:col-span-6"
        return <PremiumV3Reveal key={section.id} motionStyle={motionStyle} delay={index * 0.04} className={span}><figure><div className={`group relative overflow-hidden bg-[var(--demo-surface-alt)] ${index % 3 === 1 ? "aspect-[4/5]" : "aspect-[5/4]"}`}><PremiumV3Media media={itemMedia} className="absolute inset-0" sizes="(max-width:768px) 100vw, 58vw" /><span className="absolute left-4 top-4 bg-white/90 px-3 py-2 text-[10px] font-bold tracking-[.2em] text-black">LOOK {String(index + 1).padStart(2, "0")}</span></div><figcaption className="grid gap-3 border-b border-[var(--demo-line)] py-5 sm:grid-cols-[.45fr_1fr]"><h2 className={`${demoHeadlineClass(section.heading, "card")} font-[var(--demo-heading-weight)] [font-family:var(--demo-font-display)]`}>{section.heading}</h2><p className="text-sm leading-7 text-[var(--demo-muted)]">{section.body}</p></figcaption></figure></PremiumV3Reveal>
      })}</div></section>
