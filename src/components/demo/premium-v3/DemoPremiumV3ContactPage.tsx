...
import { PremiumV3Reveal } from "./PremiumV3Primitives"
...
export function DemoPremiumV3ContactPage({ data }: { data: DemoMultiPageData }) {
  ...
  return (
    <div className="overflow-hidden bg-[var(--demo-surface)] text-[var(--demo-ink)]">
      ...
      <section className="px-5 py-20 sm:px-10 sm:py-28 lg:px-16"><div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[.58fr_1.42fr] lg:gap-20"><PremiumV3Reveal motionStyle={motionStyle}><p className="text-xs font-bold uppercase tracking-[.3em] text-[var(--demo-accent)]">Contact form</p><h2 className={`mt-6 [font-family:var(--demo-font-display)] ${demoHeadlineClass("お問い合わせ。")}`}>お問い合わせ。</h2><p className="mt-7 text-sm leading-8 text-[var(--demo-muted)]">入力、確認、完了表示まで体験できます。非公開デモでは相手事業者への誤送信を防ぐため、外部送信を停止しています。</p></PremiumV3Reveal><PremiumV2InquiryForm data={data} previewOnly={isPreviewForm} /></div></section>
    </div>
  )
}
