...
export function DemoPremiumV3AboutPage({ data }: { data: DemoMultiPageData }) {
  ...
  return (
    <div className="overflow-hidden bg-[var(--demo-surface)] text-[var(--demo-ink)]">
      <PremiumV3PageHero title={about.title} subtitle={about.subtitle} eyebrow="OUR STORY" media={hero} recipe={data.designRecipe} />
      ...
      <section className="grid bg-[var(--demo-ink)] text-white lg:grid-cols-[1.1fr_.9fr]">
        ...
        <div className="flex items-center px-5 py-20 sm:px-10 sm:py-28 lg:px-14"><PremiumV3Reveal motionStyle={motionStyle}><p className="text-xs font-bold uppercase tracking-[.3em] text-white/42">Philosophy</p><blockquote className="mt-8 text-4xl leading-[1.14] tracking-[-.03em] sm:text-5xl [font-family:var(--demo-font-display)]">“{about.mission}”</blockquote><p className="mt-8 text-sm leading-8 text-white/58">日々の商品やサービス、空間のあり方まで、この考え方を軸にご案内しています。詳しい内容は各ページと公式情報をご確認ください。</p></PremiumV3Reveal></div>
      </section>

      <section className="bg-[var(--demo-surface-alt)] px-5 py-20 sm:px-10 sm:py-28 lg:px-16 lg:py-32"><div className="mx-auto max-w-7xl"><PremiumV3Reveal motionStyle={motionStyle} className="mb-8 grid gap-6 border-b border-[var(--demo-line)] pb-9 md:grid-cols-[1fr_.55fr] md:items-end"><div><p className="text-xs font-bold uppercase tracking-[.3em] text-[var(--demo-muted)]">What we value</p><h2 className="mt-5 text-4xl tracking-[-.035em] sm:text-6xl [font-family:var(--demo-font-display)]">大切にしていること。</h2></div><p className="text-sm leading-8 text-[var(--demo-muted)]">確認できる事実と、事業者が大切にする考え方を分けて、誠実にお伝えします。</p></PremiumV3Reveal><div className="divide-y divide-[var(--demo-line)]">{about.values.map((value, index) => <PremiumV3Reveal key={value.title} motionStyle={motionStyle} delay={index * 0.05} className="grid gap-5 py-8 md:grid-cols-[90px_.6fr_1fr]"><span className="text-sm text-[var(--demo-muted)]">0{index + 1}</span><h3 className="text-3xl [font-family:var(--demo-font-display)]">{value.title}</h3><p className="text-sm leading-8 text-[var(--demo-muted)]">{value.description}</p></PremiumV3Reveal>)}</div></div></section>

      <section className="px-5 py-20 sm:px-10 sm:py-28 lg:px-16 lg:py-32"><div className="mx-auto max-w-7xl"><PremiumV3Reveal motionStyle={motionStyle} className="mb-10"><p className="text-xs font-bold uppercase tracking-[.3em] text-[var(--demo-accent)]">Scenes</p><h2 className="mt-5 text-4xl tracking-[-.035em] sm:text-6xl [font-family:var(--demo-font-display)]">{data.companyName}の日々。</h2></PremiumV3Reveal><PremiumV3MediaCarousel media={media} label={`${data.companyName}のイメージギャラリー`} /><div className="mt-10 flex flex-wrap gap-5">{instagram && <a href={instagram.href} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-3 border-b border-[var(--demo-ink)] pb-2 text-sm font-bold">最新情報をInstagramで見る<ArrowUpRight className="h-4 w-4" /></a>}<a href={`${basePath}/services`} className="inline-flex items-center gap-3 border-b border-[var(--demo-line)] pb-2 text-sm font-bold">{data.meta.navLabels?.services ?? "サービス"}を見る<ArrowRight className="h-4 w-4" /></a></div></div></section>
    </div>
  )
}
