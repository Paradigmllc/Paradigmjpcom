...
import { AlertTriangle, ArrowDown, CheckCircle2, FileSpreadsheet, LoaderCircle, ShieldCheck, Sparkles, Upload } from "lucide-react"
        ...
        <section ref={resultRef} className="scroll-mt-24 bg-white px-5 py-16 sm:px-8 lg:py-24" aria-labelledby="diagnosis-result-title">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-14 grid gap-8 rounded-3xl bg-slate-950 p-6 text-white sm:p-10 lg:grid-cols-[0.8fr_1.2fr] lg:items-center">
              <div><CheckCircle2 className="text-emerald-400" aria-hidden="true" /><h2 className="mt-5 text-2xl font-bold">次は14日間、実案件で検証</h2><p className="mt-4 text-sm leading-7 text-slate-300">既存システムは置き換えず、回収候補・担当・次回アクション・接触履歴だけを運用します。自動課金はありません。</p><p className="mt-5 flex items-center gap-2 text-xs font-semibold text-violet-300"><ArrowDown aria-hidden="true" />診断値は申込に自動反映されます</p></div>
              <div className="rounded-2xl bg-white p-5 text-slate-950 sm:p-7"><PilotForm diagnosis={diagnosis} /></div>
            </div>
          </div>
        </section>
      )}
