...
import { Factory, RefreshCw, SearchCheck, ShieldCheck } from "lucide-react"
...
export function LeadFactoryConsole() {
  ...
  const loadRun = useCallback(async (runId: string) => {
    ...
    try {
      const response = await fetch(`/api/sales/lead-candidates/runs/${runId}`, { cache: "no-store" })
      const payload = await response.json() as { ok?: boolean; recentItems?: RunItem[]; error?: string }
      if (!response.ok || !payload.ok) throw new Error(payload.error ?? "ラン詳細を取得できませんでした")
