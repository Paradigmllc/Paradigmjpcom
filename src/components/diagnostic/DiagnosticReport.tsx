  ...
  return (
    <article className="rounded-lg border border-zinc-200 bg-white p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3">
        ...
        <Pill tone={tone}>{pain.severity}</Pill>
      </div>
  ...
  return (
    <div className="grid gap-3 border-t border-zinc-200 py-4 first:border-t-0 md:grid-cols-[180px_minmax(0,1fr)_120px]">
      ...
      <div className="md:text-right">
        <Pill tone={sourceTone(item.score)}>{item.status}</Pill>
      </div>
    </div>
  )
}
      ...
      <main>
        ...
        <section className="px-5 pb-12">
          <div className="mx-auto grid max-w-6xl border-y border-zinc-200 md:grid-cols-4">
            <Stat label={copy.evidenceReady} value={`${data.source_coverage.collected}`} detail={`${data.source_coverage.configured} configured / ${data.source_coverage.missing} missing`} />
        ...
        <DarkDiagnosticSurface data={data} copy={copy} confidence={confidence} lang={lang} />
          ...
          <div className="mx-auto grid max-w-6xl gap-8 lg:grid-cols-[minmax(0,1fr)_360px]">
            <div>
              ...
              <div className="mt-6 grid gap-4 md:grid-cols-2">
                {data.intelligence.painPoints.map((pain) => <PainCard key={pain.id} pain={pain} copy={copy} />)}
              </div>
            </div>
        ...
        <section className="px-5 py-14">
          <div className="mx-auto max-w-6xl">
            ...
            <div className="mt-7 grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {data.intelligence.signals.slice(0, 9).map((signal) => <SignalCard key={signal.id} signal={signal} copy={copy} />)}
            </div>
            <div className="mt-8 rounded-lg border border-zinc-200 bg-white px-5 shadow-sm">
              {visibleSources.map((item) => <SourceRow key={item.slug} item={item} copy={copy} />)}
            </div>
          </div>
        </section>
