...
export function SectionRender({ block: b }: { block: AnyBlock }) {
  const align = (b.alignment as string) ?? "center"
  const bgKey = (b.background as string) ?? "default"
  const bg = bgKey === "surface" || bgKey === "accent-soft" ? "bg-paradigm-paper-deep" : "bg-paradigm-paper"
  ...
  return (
    <section className={`${bg} paradigm-section relative overflow-hidden`}>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }}
        ...
        <h2 className="font-display text-[32px] md:text-[52px] leading-[1.15] tracking-[-0.01em] text-paradigm-ink mb-6">{String(b.title ?? "")}</h2>
        {!!b.subtitle && <p className="text-[15px] md:text-[17px] text-paradigm-ink-soft max-w-3xl mx-auto leading-[1.85]">{String(b.subtitle)}</p>}
      </motion.div>
    </section>
  )
}

export function CTARender({ block: b }: { block: AnyBlock }) {
    ...
    <section className="relative bg-paradigm-ink paradigm-section overflow-hidden">
      ...
      <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, margin: "-80px" }}
        ...
        <h2 className="font-display text-[32px] md:text-[52px] leading-[1.1] tracking-[-0.015em] mb-6">
          <span className="bg-gradient-to-r from-paradigm-paper via-paradigm-glow to-paradigm-tech bg-clip-text text-transparent bg-[length:200%_100%] animate-[gradientShift_5s_ease_infinite]">{String(b.title ?? "")}</span>
        </h2>
        {!!b.subtitle && <p className="text-[15px] md:text-[17px] text-paradigm-paper/70 max-w-xl mx-auto mb-10 leading-[1.85] font-light">{String(b.subtitle)}</p>}
        <div className="flex flex-wrap gap-4 justify-center">
          {primary?.href && <a href={primary.href} className="inline-flex items-center gap-2 bg-paradigm-glow/20 backdrop-blur-sm border border-paradigm-glow/40 text-paradigm-paper hover:bg-paradigm-glow/30 px-8 py-4 text-[12px] tracking-[0.18em] uppercase transition-all paradigm-glow-sm rounded-xl">{primary.label ?? "Get started"} <ArrowRight size={16} /></a>}
          {secondary?.href && <a href={secondary.href} className="inline-flex items-center gap-2 border border-paradigm-paper/15 text-paradigm-paper/70 hover:bg-paradigm-paper/8 px-8 py-4 text-[12px] tracking-[0.18em] uppercase transition-colors rounded-xl">{secondary.label ?? "Learn more"}</a>}
        </div>
      </motion.div>
    </section>
  )
...
export function FAQRender({ block: b }: { block: AnyBlock }) {
  ...
  return (
    <section className="bg-paradigm-paper-deep paradigm-section relative overflow-hidden">
      ...
      <div className="max-w-3xl mx-auto px-6 md:px-12 relative z-10">
        {!!b.title && <h2 className="font-display text-[32px] md:text-[44px] leading-[1.15] tracking-[-0.01em] text-paradigm-ink text-center mb-3">{String(b.title)}</h2>}
        {!!b.subtitle && <p className="text-[15px] text-paradigm-ink-soft text-center mb-12 leading-[1.85]">{String(b.subtitle)}</p>}
            ...
            <motion.li key={i} initial={{ opacity: 0, y: 8 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ duration: 0.3, delay: i * 0.05 }}>
              <div className="border-b border-paradigm-line">
                <details className="group">
                  ...
                  {item.answer && <div className="pl-1 pr-8 pb-7 -mt-2 text-[14px] md:text-[15px] text-paradigm-ink-soft leading-[1.85]"><RichText data={item.answer} /></div>}
                </details>
              </div>
            </motion.li>
          ))}
...
export function RichTextRender({ block: b }: { block: AnyBlock }) {
  ...
  return (
    <section className="bg-paradigm-paper paradigm-section">
      <div className={`${widthClass} mx-auto px-6 md:px-12 prose prose-paradigm max-w-none`}><RichText data={content} /></div>
    </section>
  )
}

export function SplitContentRender({ block: b }: { block: AnyBlock }) {
    ...
    <section className={`${bg} paradigm-section relative overflow-hidden`}>
      <div className="max-w-6xl mx-auto px-6 md:px-12 grid grid-cols-1 md:grid-cols-2 gap-12 md:gap-16 items-center">
        ...
        <div className={reverse ? "md:order-1" : ""}>
          {image?.url ? <img src={image.url} alt={image.alt ?? ""} className="rounded-2xl paradigm-glass w-full" loading="lazy" /> : <div className="bg-paradigm-line/20 rounded-2xl aspect-square flex items-center justify-center paradigm-eyebrow text-paradigm-ink-mute">{imagePlaceholder}</div>}
        </div>
      </div>
    </section>
  )
