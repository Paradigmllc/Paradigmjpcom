...
export function VideoRender({ block: b }: { block: AnyBlock }) {
...
export function MarqueeRender({ block: b }: { block: AnyBlock }) {
  ...
  const reverse = (b.direction as string) === "right"
  return (
    <section className="bg-paradigm-paper-deep py-7 overflow-hidden border-y border-paradigm-line relative">
      ...
      <Marquee duration={duration} pauseOnHover reverse={reverse} className="relative z-10">
        {items.map((it, i) => (
...
export function LogoCloudRender({ block: b }: { block: AnyBlock }) {
    ...
    <section className="bg-paradigm-paper-deep paradigm-section py-16 border-y border-paradigm-line">
      <div className="max-w-6xl mx-auto px-6 md:px-12 text-center">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-6 gap-8 items-center justify-items-center opacity-30 hover:opacity-50 transition-opacity grayscale">
          {logos.map((l, i) => (
            <div key={i} className="paradigm-eyebrow text-paradigm-ink-mute text-[11px] tracking-[0.14em] uppercase">{l.alt ?? `Logo ${i + 1}`}</div>
          ))}
        </div>
      </div>
    </section>
