...
export default function BlockRendererHero(b: AnyBlock) {
  ...
  if (!isDark) {
    return (
      <section className="relative bg-paradigm-paper paradigm-section pt-36 text-center">
        <div className="max-w-5xl mx-auto px-6 md:px-12">
          {!!b.badge && <p className="paradigm-eyebrow text-paradigm-accent mb-6">{String(b.badge)}</p>}
          <h1 className="font-display text-[40px] md:text-[72px] leading-[1.08] tracking-[-0.015em] text-paradigm-ink mb-6">
            {String(b.title ?? "")}
          </h1>
          {!!b.subtitle && <p className="text-[15px] md:text-[17px] text-paradigm-ink-soft max-w-2xl mx-auto leading-[1.85] mb-10">{String(b.subtitle)}</p>}
          <div className="flex flex-wrap gap-3 justify-center">
            {primary?.href && <a href={primary.href} className="inline-flex items-center gap-2 bg-paradigm-ink text-paradigm-paper px-8 py-4 text-[12px] tracking-[0.18em] uppercase hover:bg-paradigm-accent transition-colors">{primary.label ?? "Learn more"}</a>}
            {secondary?.href && <a href={secondary.href} className="inline-flex items-center gap-2 border border-paradigm-line text-paradigm-ink-soft hover:border-paradigm-ink hover:text-paradigm-ink px-8 py-4 text-[12px] tracking-[0.18em] uppercase transition-colors">{secondary.label ?? "More"}</a>}
          </div>
        </div>
      </section>
    )
    ...
    <section className="relative min-h-[80vh] sm:min-h-[90vh] flex items-center bg-paradigm-ink overflow-hidden">
      ...
      <div className="relative z-10 w-full max-w-5xl mx-auto px-6 md:px-12 pt-32 pb-20 text-center">
        <motion.div initial={{ opacity: 0, y: -20 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.9, ease: EASE }}>
          ...
          {!!b.subtitle && <p className="text-[15px] md:text-[17px] text-paradigm-paper/70 max-w-2xl mx-auto mb-10 leading-[1.85] font-light">{String(b.subtitle)}</p>}
