...
function productFromRow(row: DbRow): ShopifyOpsProduct {
  ...
  const supplierUrl = nullableStringFrom(row.supplier_url)
  const primaryImageUrl = nullableStringFrom(row.primary_image_url)
  const originCountryCode = nullableStringFrom(row.origin_country_code)
  const hsCode = nullableStringFrom(row.hs_code)
  const fulfillmentDays = numberFrom(row.fulfillment_days)
  const supplierVerified = row.supplier_verified === true
  const sampleVerified = row.sample_verified === true
  const imageRightsVerified = row.image_rights_verified === true
  const complianceVerified = row.compliance_verified === true
  const fulfillmentVerified = row.fulfillment_verified === true
  const publicationGate = evaluateProductPublicationGate({
    status: stringFrom(row.status), inventoryOnHand: numberFrom(row.inventory_on_hand), photoReady: numberFrom(row.photo_ready),
    shopifyHandle: nullableStringFrom(row.shopify_handle), supplierUrl, primaryImageUrl, originCountryCode, hsCode,
    fulfillmentDays, supplierVerified, sampleVerified, imageRightsVerified, complianceVerified, fulfillmentVerified,
  })
...
function contentFromRow(row: DbRow): ShopifyOpsContentItem {
  return {
    ...
    caption: nullableStringFrom(row.caption),
    mediaUrl: nullableStringFrom(row.media_url),
    destinationUrl: nullableStringFrom(row.destination_url),
    autoGenerated: row.auto_generated === true,
    generationDate: nullableStringFrom(row.generation_date),
    ...
    lastPublishAttemptAt: nullableStringFrom(row.last_publish_attempt_at),
    externalPostId: nullableStringFrom(row.external_post_id),
  }
}
...
export async function updateShopifyOpsProduct(input: UpdateProductInput): Promise<ShopifyOpsProduct> {
  ...
  const { data, error } = await database
    ...
    .update({
      ...
      fulfillment_days: input.fulfillmentDays,
      supplier_verified: input.supplierVerified,
      sample_verified: input.sampleVerified,
      image_rights_verified: input.imageRightsVerified,
      compliance_verified: input.complianceVerified,
      fulfillment_verified: input.fulfillmentVerified,
