...
Paradigm addresses these items through our Japan Entry Package, which validates the opportunity and addresses the named customer-path gap. The package is $15,000 paid upfront, and the first six months of managed support are included at no additional monthly charge. Would a detailed Japan opportunity analysis be useful?`,
]
...
Paradigm addresses these items through our Japan Entry Package, which validates the opportunity and addresses the named customer-path gap. The package is $15,000 paid upfront, with the first six months of managed support included at no additional monthly charge. Would you like a detailed Japan opportunity analysis?`
  ...
  it("rejects the prior generic rank-led pattern", () => {
    ...
    const review = reviewPersonalizedJapanEntryMessage({
      message:
        "Hi Example team — I noticed your site has a Tranco rank of 52,000. Given that reach, Japan is a logical next step. Our Japan Entry Package is $15,000 paid upfront, with the first six months included. Is this relevant?",
      companyName: "Example",
      productContext,
      productEvidence,
  ...
  it("accepts a 92+ editorial draft only after every dimension and hard gate pass", async () => {
    const caller = vi
      .fn()
      .mockResolvedValueOnce(generationResponse())
      .mockResolvedValueOnce(
      ...
      .mockResolvedValueOnce(repairResponse(messages[1]))
      .mockResolvedValueOnce(criticResponse({ selected_index: 0, scores: { specificity: 23, naturalness: 23, credibility: 23, executive_relevance: 23 } }))
      .mockResolvedValueOnce(repairResponse(messages[1]))
    ...
    const result = await generatePersonalizedJapanEntryMessage(generateInput(), caller)
    expect(result.ok).toBe(true)
    expect(result.review).toMatchObject({ passed: true, score: 92 })
