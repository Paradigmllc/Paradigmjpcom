...
const CHRONOLOGY = /(?:19|20)\d{2}/u
const HISTORY = /(長年|創業|以来|歩んできた|信頼を築いて)/u
...
export function findUnsupportedDemoClaims(copy: string, verifiedFacts: string): UnsupportedDemoClaim[] {
  ...
  if (CHRONOLOGY.test(copy) && !CHRONOLOGY.test(verifiedFacts)) claims.push("chronology")
  if (HISTORY.test(copy) && !/(創業|設立|沿革|(?:19|20)\d{2})/u.test(verifiedFacts)) claims.push("history")
