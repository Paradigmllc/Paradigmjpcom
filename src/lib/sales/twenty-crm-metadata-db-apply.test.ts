...
describe("normalizeTwentyCompanyViewsViaDatabase", () => {
  it("persists visibility and position in application-owned universal overrides", () => {
    ...
    expect(source).toContain("operand in ('IS', 'CONTAINS')")
    expect(source).toContain("values (gen_random_uuid(), $1, 'CONTAINS'")
    expect(source).not.toContain("values (gen_random_uuid(), $1, 'IS'")
  })
})
