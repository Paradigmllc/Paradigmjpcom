...
beforeEach(() => {
  ...
  mocks.corpus
    .mockResolvedValueOnce({
      ...
      sourceByDomain: { "store-one.us": ["zone_file"] },
      sourceStats: [{ source: "zone_file", pattern: "*.us", fetched: 1, total: 5597, ok: true }],
      failures: [],
    })
    .mockResolvedValueOnce({
      ok: true,
      domains: ["store-two.com"],
      sourceByDomain: { "store-two.com": ["passive_domain_feed"] },
      sourceStats: [{ source: "passive_domain_feed", pattern: "*.com", fetched: 1, total: 600000, ok: true }],
      failures: [],
    })
})
