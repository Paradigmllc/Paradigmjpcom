...
export async function checkSlidevGotenbergHealth(): Promise<ServiceHealthResult> {
  ...
  try {
    ...
    const [resSlidev, resGotenberg] = await Promise.all([
      fetch(slidev.toString(), { signal: AbortSignal.timeout(10_000) }).catch((e) => { console.warn("[oss-health-infra] slidev health fetch failed:", e); return { ok: false, status: 0 } }),
      fetch(gotenberg.toString(), { signal: AbortSignal.timeout(10_000) }).catch((e) => { console.warn("[oss-health-infra] gotenberg health fetch failed:", e); return { ok: false, status: 0 } }),
    ])
    return {
