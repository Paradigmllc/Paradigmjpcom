...
export function manualWorkEligibility(input: {
  ...
  if (input.profile.smbStatus === "rejected") reasons.push("Company is outside the overseas SMB scope")
  else if (input.profile.smbStatus !== "qualified" || input.profile.smbConfidence < 70) reasons.push("SMB classification needs review")
  if (input.profile.japanEntryFitStatus === "rejected") reasons.push("Company is outside the current Japan Entry offer scope")
  else if (input.profile.japanEntryFitStatus !== "qualified" || input.profile.japanEntryFitConfidence < 70) reasons.push("Japan Entry fit needs review")
...
export function manualWorkTerminalStatus(
  profile: ManualCompanyProfile,
  eligible: boolean,
): Extract<ManualWorkStatus, "completed" | "needs_review" | "rejected"> {
  if (
    profile.isJapaneseCompany
    || profile.countryCode === "JP"
    || profile.smbStatus === "rejected"
    || profile.japanEntryFitStatus === "rejected"
  ) return "rejected"
  return eligible ? "completed" : "needs_review"
}
