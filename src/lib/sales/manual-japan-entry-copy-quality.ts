...
const MALFORMED_ENGLISH = /(?:\bFor the (?:Enable|Build|Create|Get|Make|Allows?|Lets?)\b|\bthe (?:enable|allows?|lets?) (?:long|lower|better|faster)\b|\b(?:a|an) [A-Z]{2,}\s+(?:are|were)\b|\b(?:it|this|that) (?:are|were)\b|\b(?:that|where|because|as) (?:Customers|Users|Teams|Developers|Merchants|Buyers)\b)/
const STOCK_BODY = /(?:the public-page review leaves the .{0,80} decision unverified|the next decision is not a full launch|a concise brief can stay within the verified product scope|the evidence can be organized into what the pages establish|so your team can assess that step with evidence|the surfaces an evaluator would use .{0,120} are not present|leaves (?:one|a) concrete (?:question|decision) open|the output would be a clear basis for)/i
const STOCK_CTA = /(?:I can share a detailed Japan opportunity analysis based on this public evidence|Could you forward (?:this|it) to (?:the )?founder or (?:the )?person responsible for international growth)/i
...
export function reviewManualMessagePersonalization(input: {
  ...
  const templatePattern = STOCK_BODY.test(body) || STOCK_CTA.test(final)
