...
export const JAPAN_ENTRY_GENERATION_SYSTEM_PROMPT = [
  ...
  "Paragraph 4 must begin with this exact sentence and must not add unprovided deliverables: 'Paradigm addresses these items through our Japan Entry Package, which validates the opportunity and addresses the named customer-path gap.' Then state the commercial term: $13,000 paid upfront, with the first six months of managed support included at no additional monthly charge. End with exactly one low-pressure yes/no question offering a detailed Japan opportunity analysis. Do not offer both a report and a call.",
...
export function initialInterestGenerationPrompt(
  ...
  return [
    ...
    "Every sentence must add a distinct point. Never repeat or lightly rephrase the product description, public-page finding, decision implication, or CTA in another sentence. In repair mode, delete the weaker duplicate instead of substituting synonyms.",
  ...
  const system = [
    ...
    purpose === "initial_interest"
      ? initialInterestOptions.includePrice
        ? "Require only the exact $13,000 fixed launch fee and six included managed-support months. Reject other price, scarcity, continuation pricing, URL, attachment, booking link, call offer, or a claim that a report already exists."
