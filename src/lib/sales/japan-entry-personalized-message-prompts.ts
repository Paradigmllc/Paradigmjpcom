...
export function initialInterestGenerationPrompt(
  ...
  return [
    ...
    `The personalized body, excluding the greeting and signature, must be ${options.includePrice ? "145-210" : "75-190"} English words and contain three to five short paragraphs separated by a blank line (\\n\\n). The first body paragraph is exactly one sentence containing only a grounded product observation; never add a second opening sentence. The final body paragraph is the permission or routing CTA. Place the public-page Japan finding and company-specific decision implication naturally in the middle; they may be combined or ordered differently. Aim for ${options.includePrice ? "165-185" : "95-125"} body words, but never add an unsupported claim or reusable process sentence merely to reach the target. Before returning JSON, count whitespace-delimited words in the body and rewrite until the count is inside the required range. Do not use headings, bullets, or Markdown.`,
    ...
    `The final body paragraph, immediately before the signature, must explain what the Japan opportunity analysis would help this company decide, using copy_plan.solutionFocus without copying it mechanically, and end with exactly one original permission or routing question. Repeat the same natural shortened grounded product subject used in the middle and name the actual test the analysis would define; do not replace it with 'this workflow', 'that workflow', or 'the workflow'. Generic labels such as 'current market-readiness question', 'testable entry decision', 'market-entry question', or 'entry-readiness decision' are invalid. The question itself must express the required_question_decision_anchor meaning by naming the concrete decision, customer path, evaluation, purchase, localization, positioning, readiness, validation, or what to test; use natural wording rather than mechanically copying the label. A generic 'would you like it?' or 'could you forward this to the founder/person responsible for international growth?' is invalid. Required CTA meaning: ${ctaMeaning} Satisfy every required_cta_contract field, include the required company/product anchor once and the exact customer-path anchor once, and return cta_type as exactly one of permission_to_send, right_person, or founder_forward. Use founder_forward only for an explicit founder/international-growth forwarding request, right_person for owner routing, and permission_to_send for permission to send the analysis. Do not reuse an approved stock paragraph. Never write 'I can share a detailed Japan opportunity analysis based on this public evidence'. Do not offer both a report and a call.`,
