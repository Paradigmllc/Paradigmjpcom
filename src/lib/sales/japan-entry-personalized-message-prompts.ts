...
export const JAPAN_ENTRY_GENERATION_SYSTEM_PROMPT = [
  ...
  "Paragraph 4 must begin with this exact sentence and must not add unprovided deliverables: 'Paradigm addresses these items through our Japan Entry Package, which validates the opportunity and addresses the named customer-path gap.' Then state the commercial term: $15,000 paid upfront, with the standard $2,000/month managed-operation layer included for 90 days at no additional monthly fee ($2,000/month × 3 months = $6,000 of value) for selected launch partners. End with exactly one low-pressure yes/no question offering a detailed Japan opportunity analysis. Do not offer both a report and a call.",
...
export function initialInterestGenerationPrompt(
  ...
  return [
    "You write concise, natural B2B inquiry-form messages to founders and senior decision-makers at overseas SMBs.",
    "Return JSON only. For generate_candidates return {strategy:{primary_observation,why_now,japanese_segment,japan_gap,opportunity_angle,offer_relevance,tone,cta,country_adaptation,prohibited_claims},candidates:[{message,fact_ids,product_evidence,product_evidence_rendering,angle,opening_style,diagnostic_focus,cta_type,architecture,personalization_anchors,solution_focus},...]}. prohibited_claims must be a JSON array of 3-8 short strings, each under 120 characters, never one combined string. Return one to three candidates, and include an alternative only when its reasoning and structure are materially different. For repair_candidate return {candidate:{message,fact_ids,product_evidence,product_evidence_rendering,angle,opening_style,diagnostic_focus,cta_type,architecture,personalization_anchors,solution_focus}}.",
    "Build the strategy before drafting. Connect a supplied company observation to the exact public-page gap, the product-path decision a Japan opportunity analysis would inform, and a low-friction permission or routing CTA. Do not require a Japanese customer-segment hypothesis. When the payload does not verify a segment, set japanese_segment to 'Unverified' and refer to the product or customer path, never a Japanese audience. Every strategy field is subject to the same evidence limits as the message: when the payload does not verify demand, underserved status, discoverability, evaluation behavior, or effect, write 'Unverified' rather than inventing it.",
    ...
    `The personalized body, excluding the greeting and signature, must be ${options.includePrice ? "145-210" : "80-190"} English words and contain four to five short paragraphs separated by a blank line (\\n\\n). The first body paragraph is exactly one sentence containing only a grounded product observation. Put the audited public-page absence in its own one-sentence paragraph. Follow it with a separate company-specific decision paragraph that repeats a natural shortened grounded product subject. The final body paragraph is the permission or routing CTA. Aim for ${options.includePrice ? "165-185" : "105-135"} body words, but never add an unsupported claim or reusable process sentence merely to reach the target. Before returning JSON, count whitespace-delimited words in the body and rewrite until the count is inside the required range. Do not use headings, bullets, or Markdown.`,
    ...
    "Open directly with the observable company detail. The first body paragraph must be exactly one sentence containing the exact company_name and product_evidence_rendering verbatim. Describe the product directly; never say the company 'publicly describes its offering with the phrase', 'publicly describes its offering around', or otherwise narrate your evidence extraction. Do not add a second product sentence. When supplemental_product_evidence is non-null, use it only if it fits naturally inside the same factual sentence without repeating the primary evidence; otherwise omit it, especially when it reads like a button, command, or conversion CTA. Keep this paragraph free of Japan claims, audit gaps, estimates, buyer behavior, demand, outcomes, praise, or sender biography. Do not invent a generic user scenario such as 'A retailer using [company] can...'. Do not begin with I noticed, I came across, I was impressed, I am reaching out, I wanted to reach out, hope this message finds you well, or another reusable prospecting opener.",
    "Return required_product_evidence exactly and unchanged as product_evidence. It describes a real capability, workflow, product category, or customer use; do not conjugate, shorten, or broaden the source field. Return product_evidence_rendering as natural English that faithfully preserves that source phrase while remaining neutral and declarative, with no added fact, outcome, customer claim, or interpretation, and use the rendering verbatim in the first body paragraph. When the source is already declarative or a noun phrase, product_evidence_rendering must equal it exactly. If the source phrase is a marketing imperative such as 'Explore...', 'Convert...', or 'Build...', nominalize only the leading verb into a grammatical capability phrase and preserve every object and qualifier; never embed the capitalized command after can, may, or to. Never carry Book a consultation, Get started, Request a demo, or another site CTA into either the rendering or message. Never copy hidden text, responsive-display instructions, cookie text, navigation labels, or other UI residue into the rendering. When product_names is non-empty, mention at least one supplied product name exactly in the personalized body. Mention at most two supplied capabilities. Do not invent customer outcomes, needs, demand, or Japan applicability.",
    "The middle body paragraph or paragraphs must state the selected public-page Japan finding separately from the product observation and explain the exact decision that remains unverified for this company and product. Write the audited absence as a standalone sentence; never continue it with a comma plus so, therefore, or which. The audit proves only what the checked public pages did not show; never restate it as the workflow, product, capability, offering, or experience actually lacking a path. A missing Japanese-language customer path does not prove that an interface, experience, onboarding, documentation, UI, checkout, support, or another surface is English-only, absent, untested, or in Japanese; do not name any such surface unless required_facts names it. Do not label an audience as Japanese developers, buyers, users, customers, teams, or evaluators unless a required fact names that exact segment, and never refer vaguely to 'that audience'. Name the actual grounded workflow in natural shortened form, such as 'the screenshot-to-code workflow' or 'customer-behaviour analysis', when framing whether it should be tested through a Japanese-language path before wider localization is considered. Build that noun phrase from supplied objects and product terms; never invent a hyphen-chained verb label such as 'integrate-and-explore workflow'. Do not write 'the product's documented capability', 'the documented capability', or refer generically to the current, documented, or verified product scope. Do not claim that non-English evaluators can still use the product, predict how far evaluators progress, or connect the finding to trial completion, conversion, adoption, readiness, demand, buyer behavior, causation, or loss.",
    "Every sentence must add a distinct point. Never repeat or lightly rephrase the product description, shortened product subject, public-page finding, decision implication, CTA, evidence disclaimer, or a 'before wider/broader localization' boundary in another sentence. After the opening, use the shortened product subject no more than twice, including the final question, and never repeat any meaningful two-word product phrase three times. Use plain 'single test' language and no validation-program jargon. State the localization boundary once in the decision paragraph or CTA, not both. Never call the grounded product subject 'this workflow', 'that workflow', or 'the workflow'; name it naturally only where needed. State the public-page fact directly; avoid stock next-step transitions and write the unresolved product decision directly. Do not invent evaluators or another audience in Japan, technical fit, market fit, first impressions, market reception, or another test outcome. Never use 'product evaluation and Japanese positioning' as the analysis focus; name the grounded workflow and exact path decision. Use the exact company name no more than twice in the personalized body: once in the grounded opening and at most once in the final CTA paragraph. After that, use natural pronouns such as your product, your team, it, or the analysis. Never form an awkward possessive such as a multi-word product name followed by 's. In repair mode, delete the weaker duplicate instead of substituting synonyms.",
    "Offer or describe our Japan opportunity analysis exactly once, in the final body paragraph. Earlier paragraphs must not refer to 'the analysis', our brief, our snapshot, scope, or output; they state only the product-specific decision. The word analysis may appear earlier only when it is part of the company's grounded product capability, such as customer-behaviour analysis. Use product_evidence_rendering exactly once, in the opening; the CTA must not repeat or compactly restate it. Never use generic evidence-separation, scope, assumption, or validation-process scaffolding. Write the actual product-specific decision instead.",
...
export function criticMessages(
  ...
  const system = [
    ...
    purpose === "initial_interest"
      ? "Specificity requires exact product evidence, one supplied exact product name when available, and company-specific public-page Japan evidence. Naturalness requires a readable four-to-five-paragraph personalized body inside the exact company greeting and Tomohiro H sender signature, with the audit observation and grounded product decision in separate paragraphs plus a light permission-based close immediately before the signature. Credibility requires no unsupported inference. Executive relevance requires a concrete reason to accept the offered analysis."
    ...
    "Penalize generic praise, evidence-meta openings, vague product references, mechanical metric insertion, repeated product noun phrases, dense disclaimers, unsupported inference, abrupt pricing, validation-program jargon, stock next-step transitions, and sales clichés.",
    ...
    purpose === "initial_interest"
      ? initialInterestOptions.includePrice
        ? "Require only the exact $15,000 fixed launch fee and the selected-partner campaign: $2,000/month × 3 months = $6,000 of managed-operation value included. Reject other price, numeric scarcity, continuation pricing, URL, attachment, booking link, call offer, or a claim that a report already exists."
      ...
      : "The $15,000 upfront price and properly labeled public-signal estimates are required terms, not risk flags.",
    purpose === "initial_interest"
