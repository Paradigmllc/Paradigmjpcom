...
export const JAPAN_ENTRY_GENERATION_SYSTEM_PROMPT = [
  ...
  "Paragraph 4 must begin with this exact sentence and must not add unprovided deliverables: 'Paradigm addresses these items through our Japan Entry Package, which validates the opportunity and addresses the named customer-path gap.' Then state the commercial term: $15,000 paid upfront, with the standard $2,000/month managed-operation layer included for six months at no additional monthly fee ($2,000/month × 6 months = $12,000 of value) for selected launch partners. End with exactly one low-pressure yes/no question offering a detailed Japan opportunity analysis. Do not offer both a report and a call.",
  ...
  const system = [
    ...
    purpose === "initial_interest"
      ? initialInterestOptions.includePrice
        ? "Require only the exact $15,000 fixed launch fee and the selected-partner campaign: $2,000/month × 6 months = $12,000 of managed-operation value included. Reject other price, numeric scarcity, continuation pricing, URL, attachment, booking link, call offer, or a claim that a report already exists."
