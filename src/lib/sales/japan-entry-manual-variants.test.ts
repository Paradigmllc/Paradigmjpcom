...
const introduction = "Hello, I’m Sato from Paradigm LLC in Japan. We help overseas companies enter the Japanese market."
...
describe("manual initial-interest message variants", () => {
  it("accepts the price cell only with the current fixed terms", () => {
    ...
    const message = `${introduction}\n\n${productParagraph}\n\nIn a review of the public pages, I did not find a Japanese-language customer path. This is not a finding about demand or performance; it means the customer path available for a Japan entry decision remains unverified from the pages checked.\n\n${initialInterestClose(options)}`
  ...
  it("rejects unsupported scarcity and continuation terms", () => {
    ...
    const message = `${introduction}\n\n${productParagraph}\n\nIn a review of the public pages, I did not find a Japanese-language customer path. This is not a finding about demand or performance; it means the customer path remains unverified from the pages checked.\n\n${unsafeClose}`
