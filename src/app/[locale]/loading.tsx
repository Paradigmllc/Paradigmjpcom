 ...
 * 入力: なし (locale は親 [locale]/layout.tsx の NextIntlClientProvider context から伝播)
 * 出力: ページ全幅の minimal loader (コンテンツが描画されるまで一瞬で消える)
 *
 * H ルール (3 状態): ローディング状態 — 必須実装。
 * AE-PHP-2: ラベルは messages/{locale}.json:common 経由（旧: 日本語ハードコードで
 *           全 12 locale が「少々お待ちください」になっていた）。
 */
    ...
    <div className="min-h-[60vh] flex items-center justify-center bg-paradigm-paper">
      <div className="paradigm-glass rounded-2xl px-8 py-6 paradigm-glow-md flex items-center gap-4">
        ...
        <div className="flex flex-col">
          <span className="paradigm-eyebrow text-paradigm-accent">{t("loading")}</span>
          <span className="text-[13px] text-paradigm-ink-soft">{t("loadingMessage")}</span>
        </div>
      </div>
    </div>
  )
