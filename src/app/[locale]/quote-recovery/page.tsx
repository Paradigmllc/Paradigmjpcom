...
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  ...
  return {
    title: "Quote Recovery | 放置見積の回収管理",
    description: "製造業・機械商社向け。見積CSVから放置案件と回収優先順位を60秒で可視化します。",
...
export default function QuoteRecoveryPage() {
  return (
    <main className="overflow-hidden bg-white text-slate-950">
      <section className="relative border-b border-slate-200 px-5 pb-20 pt-32 sm:px-8 lg:pb-28 lg:pt-40">
        ...
        <div className="relative mx-auto grid max-w-6xl gap-14 lg:grid-cols-[1.08fr_0.92fr] lg:items-center">
          <div>
          ...
          <div className="rounded-3xl border border-slate-200 bg-white p-5 shadow-[0_30px_80px_-35px_rgba(15,23,42,0.4)] sm:p-7">
            ...
            <div className="mt-5 space-y-3">
              {[["北陸パーツ工業", "¥8,200,000", "154日"], ["東都精機株式会社", "¥4,800,000", "84日"], ["中央オートメーション", "¥3,100,000", "154日"]].map(([company, amount, age], index) => <div key={company} className="grid grid-cols-[auto_1fr_auto] items-center gap-3 rounded-xl bg-slate-50 p-4"><span className={`flex size-8 items-center justify-center rounded-lg text-xs font-bold ${index === 0 ? "bg-rose-100 text-rose-700" : "bg-amber-100 text-amber-700"}`}>{index + 1}</span><div><p className="text-sm font-bold">{company}</p><p className="mt-1 text-xs text-slate-500">次回アクション未設定</p></div><div className="text-right"><p className="text-sm font-bold">{amount}</p><p className="mt-1 text-xs text-slate-500">{age}</p></div></div>)}
            </div>
          </div>
        </div>
      </section>

      <section className="px-5 py-14 sm:px-8"><div className="mx-auto grid max-w-6xl gap-4 sm:grid-cols-3">{[
