...
function buildDemoData(variant: string, lang: string): DiagnosticReportData {
  ...
  const variantData: Record<string, {
    ...
    meo: {
      ...
      acts: [
        { type: "pain", icon: "MAP", headline: isJa ? "地図で選ばれていない" : "Not being chosen on the map",
          body: isJa ? "Googleマップでの「渋谷 イタリアン」検索表示順位は15位。写真3枚・口コミ5件では、1位の競合（写真48枚・口コミ127件）に勝てません。検索〜来店のコンバージョン率は競合の1/7。MEO対策だけで月間120万円の売上増が見込めます。" : "Your Google Maps ranking for 'Italian Shibuya' is #15. With 3 photos and 5 reviews vs. competitor's 48 photos and 127 reviews, your search-to-visit conversion is 1/7th of theirs. MEO optimization alone could add $8,200/month.",
          metric_label: isJa ? "MEOスコア" : "MEO score", metric_value: "15/100", metric_unit: isJa ? "点" : "pts", metric_bench: isJa ? "50点以上で集客安定" : "50+ for stable traffic", severity: "critical" },
        { type: "fear", icon: "REACH", headline: isJa ? "このままでは口コミ格差が広がる一方" : "The review gap is widening daily",
          body: isJa ? "競合Trattoria Solは週1回の口コミ返信と写真更新で、月間口コミ数が毎月15%増加。御社が何もしなければ、3ヶ月後には表示順位がさらに5位下がり、月間損失は180万円に拡大します。" : "Competitor Trattoria Sol grows reviews 15%/month through weekly responses and photo updates. If you do nothing, your ranking drops 5 more positions in 3 months, expanding monthly losses to $12,300.",
          metric_label: isJa ? "口コミ増加率" : "Review growth rate", metric_value: "0%/月", metric_unit: "", metric_bench: isJa ? "月10%以上が目安" : "10%+/month target", severity: "warning" },
        { type: "hope", icon: "SNS", headline: isJa ? "45日でMEOスコア50点・来店数2倍を達成可能" : "50 MEO score + 2x visits in 45 days",
          body: isJa
            ? "弊社のMEO最適化パッケージでは、Googleビジネスプロフィール最適化・口コミ促進・写真定期更新・競合分析を一括実施。Trattoria Solは導入45日でMEOスコアが15->68点に向上し、月間来店数が3倍になりました。御社も同様の成果が可能です。"
            : "Our MEO package handles Google Business Profile optimization, review generation, photo updates, and competitor analysis. Trattoria Sol went from 15->68 MEO score in 45 days, tripling monthly visits. Same results are achievable for you.",
          metric_label: isJa ? "最短改善期間" : "Fastest improvement", metric_value: "45日", metric_unit: "", metric_bench: isJa ? "通常3ヶ月 -> 弊社なら45日" : "Typical 3mo -> 45 days with us", severity: "info" },
      ],
      totalLoss: isJa ? "¥1,200,000" : "$8,200",
    ...
    security: {
      ...
      acts: [
        { type: "pain", icon: "TRUST", headline: isJa ? "患者が予約前に離脱している" : "Patients are leaving before booking",
          body: isJa ? "Chromeの「保護されていない通信」警告により、予約フォーム到達者の73%が離脱。月間約180件の予約機会が失われています。競合のSmile Dental ClinicはSSL対応＋HSTS設定で離脱率を12%まで改善し、月商が1.8倍に。" : "Chrome's 'Not Secure' warning causes 73% abandonment at your booking form. ~180 appointments/month are lost. Competitor Smile Dental reduced abandonment to 12% with SSL+HSTS, doubling monthly revenue.",
          metric_label: isJa ? "セキュリティスコア" : "Security score", metric_value: "22/100", metric_unit: isJa ? "点" : "pts", metric_bench: isJa ? "80点以上が安全圏" : "80+ is safe", severity: "critical" },
        { type: "fear", icon: "OPS", headline: isJa ? "個人情報保護法の監査リスクが迫っている" : "Privacy law audit risk is approaching",
          body: isJa ? "2026年4月の改正個人情報保護法では、医療関連サイトのSSL対応が実質義務化。未対応の場合、最大1億円の罰金と行政指導の対象になります。同業3院がすでに対応済み。御社だけが未対応のまま放置すれば、患者信頼の決定的な低下を招きます。" : "The 2026 privacy law update effectively mandates SSL for healthcare sites. Non-compliance risks fines up to $685K and administrative sanctions. Three peer clinics have already complied. Being the only holdout will cause decisive trust loss.",
          metric_label: isJa ? "猶予期間" : "Time remaining", metric_value: isJa ? "2ヶ月" : "2 months", metric_unit: "", metric_bench: isJa ? "今月中の対応を推奨" : "Address this month", severity: "warning" },
        { type: "hope", icon: "SPEED", headline: isJa ? "2週間でフルSSL対応・監査リスク解消" : "Full SSL compliance + audit risk eliminated in 2 weeks",
          body: isJa ? "弊社のセキュリティ対策パッケージでは、SSL証明書更新・HSTS Preload・CSPヘッダー設定・WAF導入を並行実施。Smile Dentalでは導入2週間でセキュリティスコアが22->85点に改善し、予約完了率が3.2倍に。改正法対応の証明書類も完備します。" : "Our security package handles SSL cert renewal, HSTS Preload, CSP headers, and WAF deployment in parallel. Smile Dental improved from 22->85 security score in 2 weeks, with booking completion rate up 3.2x. Full compliance documentation included.",
          metric_label: isJa ? "最短納期" : "Fastest delivery", metric_value: isJa ? "2週間" : "2 weeks", metric_unit: "", metric_bench: isJa ? "通常1-2ヶ月 -> 弊社なら2週間" : "Typical 1-2mo -> 2 weeks with us", severity: "info" },
      ],
      totalLoss: isJa ? "¥3,600,000" : "$24,500",
    ...
    video_subscription: {
      ...
      acts: [
        { type: "pain", icon: "VIDEO", headline: isJa ? "15時間の努力が成果に結びついていない" : "15 hours of effort not converting to results",
          body: isJa ? "月間15時間のInstagram運用が、リーチ2,400・エンゲージメント率0.8%という結果に終わっています。動画コンテンツ不在が主因。競合FitBoostは週4本のショート動画でリーチ18万・エンゲージメント率6.2%を達成。御社の非効率な運用は、月間42件の入会機会損失に相当します。" : "15 hours/month of Instagram work yields only 2,400 reach and 0.8% engagement. The missing factor: video content. Competitor FitBoost achieves 180K reach and 6.2% engagement with 4 shorts/week. Your inefficient approach equals 42 lost memberships/month.",
          metric_label: isJa ? "動画コンテンツ数" : "Video content/week", metric_value: "0本", metric_unit: "", metric_bench: isJa ? "週4本以上が成長ライン" : "4+/week for growth", severity: "critical" },
        { type: "fear", icon: "REACH", headline: isJa ? "動画不在がジム存続の脅威に" : "Video absence threatens gym survival",
          body: isJa ? "フィットネス業界では、動画コンテンツを持つジムの入会率が持たないジムの3.8倍。2025年以降、Instagramのアルゴリズムは動画優先に完全移行。静止画だけの御社の投稿は、すでにフォロワーの12%にしか表示されていません。" : "In fitness, gyms with video content see 3.8x higher membership rates. Instagram's algorithm fully prioritizes video since 2025. Your photo-only posts now reach just 12% of followers.",
          metric_label: isJa ? "リーチ率" : "Reach rate", metric_value: "12%", metric_unit: "", metric_bench: isJa ? "動画投稿で40%以上" : "40%+ with video", severity: "warning" },
        { type: "hope", icon: "REACH", headline: isJa ? "月額$799で週4本のプロ動画を配信" : "$799/month for 4 pro videos/week",
          body: isJa ? "弊社の動画サブスクリプションでは、撮影・編集・投稿最適化・分析まで全て代行。FitBoostは導入1ヶ月でリーチが4.2倍、3ヶ月でフォロワー4.8倍・月間入会数42件増を達成。御社の施設と知識があれば、さらに高い成果が期待できます。" : "Our video subscription handles filming, editing, posting optimization, and analytics. FitBoost saw 4.2x reach in month 1, 4.8x followers by month 3, and 42 new members/month. Your facility and expertise can exceed these results.",
          metric_label: isJa ? "月額費用" : "Monthly cost", metric_value: "$799", metric_unit: "", metric_bench: isJa ? "内製コストの1/3以下" : "<1/3 of in-house cost", severity: "info" },
      ],
      totalLoss: isJa ? "¥1,260,000" : "$8,400",
    ...
    subsidy: {
      ...
      acts: [
        { type: "pain", icon: "DATA", headline: isJa ? "使える補助金を知らないまま期限が迫っている" : "Deadlines approaching while eligible subsidies go unclaimed",
          body: isJa ? "御社の事業内容と地域から、ものづくり補助金（最大1,000万円）・事業再構築補助金（最大8,000万円）・IT導入補助金（最大450万円）の3つに適合します。しかし申請書類の準備には通常4〜6週間かかり、今から着手してもギリギリです。同業他社の申請採択率は68%。御社も十分に採択圏内です。" : "Your business profile and location qualify for 3 major subsidies: Manufacturing (up to $68K), Business Restructuring (up to $545K), and IT Adoption (up to $30K). But applications take 4-6 weeks to prepare — starting now is already tight. Peer company approval rate is 68%. You're well within qualification range.",
          metric_label: isJa ? "適合補助金数" : "Eligible programs", metric_value: "3件", metric_unit: "", metric_bench: isJa ? "申請期限まで45日" : "45 days to deadline", severity: "critical" },
        { type: "fear", icon: "OPS", headline: isJa ? "期限切れ＝全額自己負担" : "Deadline expiry = 100% self-funded",
          body: isJa ? "申請期限まで45日。書類不備で不採択になった企業の82%は「時間不足」が原因です。また採択されても実績報告の不備で交付取消になるケースが年間14%あります。自社だけで申請した場合の採択率は31%ですが、専門家支援ありでは68%に跳ね上がります。" : "45 days to deadline. 82% of rejected applications fail due to 'insufficient time.' Even if approved, 14% annually have grants revoked due to reporting errors. Self-filed applications have 31% success rate vs. 68% with expert support.",
          metric_label: isJa ? "自己申請の採択率" : "Self-filed success rate", metric_value: "31%", metric_unit: "", metric_bench: isJa ? "専門家支援で68%" : "68% with expert support", severity: "warning" },
        { type: "hope", icon: "REACH", headline: isJa ? "採択率68%・申請から報告まで一括代行" : "68% success rate — end-to-end support",
          body: isJa ? "弊社の補助金申請代行サービスでは、適合制度の調査・事業計画書作成・申請書類一式・採択後の実績報告まで一貫してサポート。山田工業は弊社支援で申請から42日で640万円の交付決定を受け、NC工作機2台を導入。御社も同様に、最短30日で申請完了が可能です。" : "Our grant application service covers program matching, business plan writing, full application package, and post-award reporting. Yamada Industries received $44K funding decision in 42 days, acquiring 2 CNC machines. Your application can be complete in as little as 30 days.",
          metric_label: isJa ? "採択率" : "Success rate", metric_value: "68%", metric_unit: "", metric_bench: isJa ? "自己申請31% -> 弊社支援68%" : "Self 31% -> 68% with us", severity: "info" },
      ],
      totalLoss: isJa ? "¥8,200,000" : "$56,000",
    ...
    outreach: {
      ...
      acts: [
        { type: "pain", icon: "REACH", headline: isJa ? "せっかくの問い合わせを47時間も放置している" : "58 monthly inquiries — abandoned for 47 hours each",
          body: isJa ? "月間58件の問い合わせに対し、返信までの平均47時間という応答速度は、B2B業界平均（4時間）の12倍遅れです。この遅延だけで問い合わせの34%が返信前に競合へ流出。月間約12件の商談機会を失っています。1件あたりの平均契約額95万円で計算すると、月間1,140万円の機会損失です。" : "Your 47-hour average response time is 12x slower than the B2B industry average of 4 hours. This alone causes 34% of inquiries to go to competitors before you reply. You're losing ~12 sales conversations/month. At $6,500 average deal size, that's $78,000/month in lost opportunity.",
          metric_label: isJa ? "平均返信時間" : "Avg response time", metric_value: isJa ? "47時間" : "47 hrs", metric_unit: "", metric_bench: isJa ? "B2B平均: 4時間" : "B2B avg: 4 hrs", severity: "critical" },
        { type: "fear", icon: "OPS", headline: isJa ? "問い合わせの34%が返信前に離脱" : "34% of inquiries gone before you reply",
          body: isJa ? "1時間以内の返信で商談化率は62%ですが、24時間以上だと12%まで低下します。御社の47時間では理論上5%以下。さらにフォーム自動判定がないため、競合の営業・採用応募・サポート依頼が全て同じフローで処理され、優先順位付けも不可能です。" : "Sub-1-hour replies convert at 62% to sales conversations. 24+ hour replies drop to 12%. Your 47 hours means theoretically under 5%. Without form auto-classification, competitor sales inquiries, job applications, and support requests all go through the same flow with no prioritization.",
          metric_label: isJa ? "商談化率" : "Conversation rate", metric_value: isJa ? "推定5%以下" : "Est. <5%", metric_unit: "", metric_bench: isJa ? "1時間以内返信で62%" : "62% if <1hr reply", severity: "warning" },
        { type: "hope", icon: "SNS", headline: isJa ? "自動化で返信時間47時間->3分・商談+12件/月" : "47hrs -> 3min response, +12 conversations/month",
          body: isJa ? "弊社のアウトリーチ自動化パッケージでは、フォーム自動分類・AI自動返信・Slack通知・CRM連携を一括導入。導入企業の平均で返信時間が47時間->3分に短縮、商談化率が5%->48%に改善、月間商談数が12件増加しています。" : "Our outreach automation package delivers form auto-classification, AI auto-reply, Slack notifications, and CRM integration. Average client results: 47hrs->3min response time, 5%->48% conversation rate, +12 qualified conversations/month.",
          metric_label: isJa ? "改善後の返信時間" : "Response time after", metric_value: isJa ? "3分" : "3 min", metric_unit: "", metric_bench: isJa ? "商談化率48%に改善" : "48% conversation rate", severity: "info" },
      ],
      totalLoss: isJa ? "¥11,400,000" : "$78,000",
