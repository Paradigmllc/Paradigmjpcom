...
export async function GET(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const snapshot = await sb.from(DB_TABLES.SALES_LEAD_CANDIDATE_RUNS).select(fields).eq("source_slug", "evidence_first_sources").order("created_at", { ascending: false }).limit(50)
      if (snapshot.error) {
      ...
      const channel = realtime.channel("form-qualified-lead-factory-events").on("postgres_changes", {
        ...
        if (closed || changed?.source_slug !== "evidence_first_sources" || !changed.id) return
