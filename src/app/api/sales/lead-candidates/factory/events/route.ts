...
const fields = "id, country_code, technology, status, requested_limit, verify_limit, min_opportunity_score, min_smb_score, min_form_confidence, fetched_count, verified_count, scored_count, forms_checked_count, forms_qualified_count, promoted_count, twenty_synced_count, failure_count, error_message, created_at, updated_at"
...
export async function GET(req: NextRequest) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      const snapshot = await sb.from(DB_TABLES.SALES_LEAD_CANDIDATE_RUNS).select(fields).eq("source_slug", "multi_source_domains").order("created_at", { ascending: false }).limit(50)
      if (snapshot.error) {
      ...
      const channel = realtime.channel("form-qualified-lead-factory-events").on("postgres_changes", {
        ...
        if (closed || changed?.source_slug !== "multi_source_domains" || !changed.id) return
