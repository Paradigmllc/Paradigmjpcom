...
import { ingestCommonCrawlCandidates } from "@/lib/sales/lead-candidates"
...
const BodySchema = z.object({
  ...
  verifyLimit: z.number().int().min(0).max(120).optional(),
...
export async function POST(req: NextRequest) {
  try {
    ...
    const result = await ingestCommonCrawlCandidates(parsed.data)
