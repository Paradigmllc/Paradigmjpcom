...
const POSTS_PART1: SeedPost[] = [
  ...
  // AI・自動化
  { slug:"ai-business-automation-2025", ja:{title:"中小企業のAI業務自動化 — 2025年に導入すべき5つのツールと導入手順", excerpt:"Dify、n8n、DeepSeek、Make、Zapier。2025年、中小企業がコストを抑えてAI自動化を実現するためのツール選定と導入手順を、実際の業務フローに沿って解説します。", cat:"AI・自動化", read:"10分", content:`## AI自動化の現実解\n\n「AIで全部自動化」は幻想です。現実的なアプローチは人間の判断が必要な部分と自動化できる部分を分離し徐々に自動化範囲を広げていくことです。\n\n## 2025年おすすめツール5選\n\nDify: AIアプリ構築プラットフォーム。チャットボット、文章生成、データ分析をノーコードで実装可能。n8n: オープンソースのワークフロー自動化。1000以上のサービスと連携。DeepSeek V4: 高精度・低コストLLM。Context Cachingで入力コスト90%OFF。Make: 視覚的なシナリオビルダー。Zapier: 7000以上のアプリ連携。\n\n## 導入ステップ\n\n業務の棚卸しと自動化候補の洗い出し、優先度・効果試算によるロードマップ作成、PoCの小規模実施、本番展開とモニタリング、継続的な改善サイクルの確立。`}, en:{title:"AI Business Automation for SMBs — 5 Tools to Deploy in 2025", excerpt:"Dify, n8n, DeepSeek, Make, Zapier. A practical guide to cost-effective AI automation tools for SMBs in 2025, with implementation steps mapped to real business workflows.", cat:"AI & Automation", read:"10 min", content:`## The Reality of AI Automation\n\nAI automating everything is a fantasy. The realistic approach is separating human judgment from automatable tasks and gradually expanding scope.\n\n## Top 5 Tools for 2025\n\nDify: No-code AI app platform. n8n: Open-source workflow automation with 1000+ integrations. DeepSeek V4: High-accuracy, low-cost LLM with 90% input cost reduction. Make: Visual scenario builder. Zapier: 7000+ app integrations.\n\n## Implementation Steps\n\nAudit operations, build roadmap with ROI estimates, run small-scale PoC, production deployment and monitoring, establish continuous improvement cycles.`}, tags:["AI","自動化","Dify","n8n","DX","業務改善"], date:"2025-08-05" },
...
const POSTS_PART2: SeedPost[] = [
  ...
  { slug:"marketing-automation-intro", ja:{title:"MA（マーケティングオートメーション）入門 — 最小構成ではじめる顧客育成の仕組み", excerpt:"MAツールは高額という先入観を捨てましょう。n8nやMakeを使えば月額数千円からMAを構築できます。実際のワークフロー例とともに解説。", cat:"デジタルマーケティング", read:"8分", content:`## MAは高くない\n\nHubSpotやMarketoのような高額ツールを想像するかもしれませんがn8nやMakeで必要な機能のほとんどを実現できます。\n\n## 最小構成MAの4要素\n\nリード獲得: LPやフォームからのデータを自動でDBに保存。スコアリング: 閲覧・開封・DLなどの行動を自動スコア化。セグメンテーション: スコアや属性で自動リスト分け。ステップメール: セグメントに応じた最適なメールを自動配信。\n\n## n8n実装例\n\n問い合わせフォーム→n8n Webhook→Google Sheets→Slack通知。メルマガ開封者→関連ホワイトペーパー自動送付。未接触リード→自動再アプローチメール。`}, en:{title:"Marketing Automation 101", excerpt:"Forget the notion that MA tools are expensive. With n8n and Make, you can build marketing automation starting at a few dollars a month. Real workflow examples included.", cat:"Digital Marketing", read:"8 min", content:`## MA Isn't Expensive\n\nn8n and Make can deliver most of what expensive tools do.\n\n## 4 Elements of Minimal MA\n\nLead capture: auto-save form data. Scoring: auto-score behaviors. Segmentation: auto-sort leads. Drip email: auto-send optimized emails per segment.\n\n## n8n Examples\n\nContact form→Webhook→Sheets→Slack. Newsletter openers→auto-send white paper. Inactive leads→re-engagement email.`}, tags:["MA","マーケティングオートメーション","n8n","Make"], date:"2025-06-18" },
]
...
export const WORKS = [
  ...
  { slug:"dxdoctor-platform", ja:{title:"DX Doctor — 中小企業DX診断プラットフォーム", industry:"DX・コンサルティング", desc:"Webサイト解析→AI診断→補助金診断までを自動化する中小企業DX診断サービス", challenge:"補助金・DX診断の専門家不足を補う、Webサイトの公開情報だけで高精度な診断を行うAIシステムの構築", solution:"Playwrightによる自動解析、DeepSeek V4による業界別診断、Dify+n8nによるレポート自動生成。人間専門家が最終レビューのみ行うハイブリッドモデル。", metrics:"診断1件あたりの工数90%削減、月間500社の自動診断が可能に"}, en:{title:"DX Doctor — SMB DX Diagnostic Platform", industry:"DX & Consulting", desc:"Automated SMB DX diagnostic service from website analysis to subsidy assessment.", challenge:"Building an AI system for high-accuracy diagnostics from public website data.", solution:"Automated analysis via Playwright, industry diagnosis via DeepSeek V4, auto-reports via Dify+n8n.", metrics:"90% reduction in per-diagnosis effort, capacity for 500 automated diagnoses/month."}, tags:["DX","AI診断","自動化","補助金","DeepSeek"], color:"amber" as const, sort:5 },
...
export const FAQS = [
  ...
  { qJa:"SEO対策の費用はいくらですか？", qEn:"How much does SEO cost?", aJa:"月額5万円（スターター）〜40万円（エンタープライズ）の3プランです。サイト規模や目標キーワード数により最適なプランが異なります。まずは無料相談でお見積りします。", aEn:"3 plans from ¥50,000 to ¥400,000/month. Depends on site size and target keywords. Free consultation available.", catJa:"SEO対策", catEn:"SEO", sort:3 },
  ...
  { qJa:"AI導入にプログラミング知識は必要ですか？", qEn:"Do I need programming knowledge for AI?", aJa:"いいえ。Difyチャットボット構築はノーコード、n8n自動化もドラッグ＆ドロップで完結します。導入研修も含めてサポートします。", aEn:"No. Dify is fully no-code, n8n is drag-and-drop. Training included.", catJa:"AI導入", catEn:"AI Implementation", sort:13 },
