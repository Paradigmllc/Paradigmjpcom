...
| Latest release | main `a00b377b`; Coolify deployment `a10d4gys2dtni0cl1uyb37o4`; healthy container `n8i2sjiqvr2d8hrzppop2m2i-100514036775`; DB 93/93, Sales health JSON `ok:true`, Twenty HTTP 200, worker restart 0, and final release gate passed | Passed. |
| Multi-batch production DB read-back | 2/2 drain-lease columns, 3/3 queue RPCs, single-running index present, old single-active index absent, zero running/queued residual batches, zero sent violations | Passed. |
| Transactional queue-chain proof | Two rollback-only canary batches: first promotion produced running 1 / queued 1; duplicate drain claim was blocked; lease release succeeded; completion promoted the second batch and kept running 1; rollback left zero canary rows | Passed without DeepSeek, Twenty, or external-send side effects. |
