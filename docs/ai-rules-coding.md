...
**DeepSeek V4 一択。LiteLLM プロキシ経由で Pro / Flash を使い分ける。**
APIキーはグローバルメモリ管理（環境変数 `LITELLM_API_KEY`）。

| 用途 | モデル | 経路 |
|------|--------|------|
| コーディング・JSON出力・大量生成 | DeepSeek V4 Pro | LiteLLM |
| 軽量タスク・高速応答 | DeepSeek V4 Flash | LiteLLM |
| 画像・PDF・マルチモーダル | DeepSeek V4 Pro | LiteLLM |
| 複雑な推論・長文生成・アーキテクチャ設計 | DeepSeek V4 Pro | LiteLLM |
