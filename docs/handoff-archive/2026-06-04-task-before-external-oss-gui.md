...
## Codex Update - 2026-06-04 外部OSS正規GUI置き換え

- [x] Revenue OS サードメニューの「動画制作」「資料・スライド」「デモサイト管理」「診断レポート」を内部テンプレート画面ではなく外部OSS GUI入口として扱うよう修正。
- [x] Directus は `https://directus.paradigmjp.com/admin`、Keystatic は `https://keystatic.paradigmjp.com`、Supabase Studio は本番 env の URL を開く導線へ変更。
- [x] OpenMontage は公式OSSにWeb管理GUIがないため、Revenue OS内のStudio風UIと `studio/openmontage` サブドメイン rewrite を廃止し、公式OSS入口へ退避。
- [x] Verification: `npx tsc --noEmit --pretty false`, `npm test -- --run src/lib/sales/integration-registry.test.ts src/lib/sales/dify-cloud.test.ts`, `npm run build`, `git diff --check` passed.
