...
## 3.1 標準制作パイプライン（HyperFrames + ComfyUI）

販売価格はAIの実行時間ではなく、企画、ブランド設計、品質保証、修正対応、派生版、ローカライズを含む「承認可能な完成動画」に対して設定する。公開価格（Essential $1,500 / Unlimited $3,500 / Priority $5,500）はこの価値定義を前提とし、AI利用を理由に値下げしない。

制作は次の責務分離を標準とする。

1. **HyperFrames** — テキスト、商品名、UI、価格、CTA、ロゴ、字幕、ブランドカラー、レイアウト、タイムライン、最終合成。HTML/CSS/JSで決定的に再現し、同じ入力から同じ最終フレームを出力する。
2. **ComfyUI** — 背景、Bロール、雰囲気映像、演出素材など、生成による表現が有効な素材。生成結果は素材候補として扱い、最終動画へ自動採用しない。
3. **Producer / Creator** — 企画、訴求、事実確認、候補選定、ブランド判断、権利確認、字幕・音量・テンポの最終QA。標準フローは「Human-directed, AI-accelerated production」とする。

実装上の`generateProfessionalVideo`は、会社・診断レポートを解決してComfyUIの背景／Bロール／サムネイル（必要時のみアバター／動画）を並列実行し、HyperFramesの診断動画を最終レーンとして生成する。各レーンは個別結果とエラーを返し、一つの素材生成失敗で成功したレーンを破棄しない。未設定のComfyUIは成功扱いにせず、`ComfyUI is not configured`としてQA・運用画面で確認できる状態にする。

営業エンリッチメントの自動処理からこの複合レーンへ切り替える場合は、認証済みComfyUIと人手QA担当を確認したうえで`PROFESSIONAL_VIDEO_PIPELINE_ENABLED=true`を設定する。既定値は`false`で、未準備の環境では既存のHyperFrames診断動画だけを生成し、GPUジョブや追加のプロンプト費用を発生させない。

### 権利・モデル方針

- 顧客支給素材、第三者素材、フォント、音源、生成モデル、LoRAはBrand and rights registerへ記録する。
- 標準採用モデルは商用利用条件が明確なものを優先し、モデルごとのライセンス、地域制限、再配布条件を案件開始前に確認する。
- 権利確認が完了していない素材は、顧客向け最終納品へ自動採用しない。採用可否は人間が承認する。
- 有料API、撮影、高額素材、高度な3D、特急対応はService Orderで別途扱う。

### 量産と品質の境界

ブランドごとに、色、フォント、ロゴ、テロップ、CTA、トランジション、字幕、音量、アスペクト比、HyperFramesテンプレート、ComfyUIワークフローを初回に定義する。以後はJSON化したブリーフから差し替え・派生版を生成するが、最終QAを省略しない。Essentialの月10本は標準範囲（60秒以内、1言語、1構成、主アスペクト比1種）に限り、AI生成カットの試行回数や人手選定を無制限とはしない。
