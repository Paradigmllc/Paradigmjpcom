...
- The video pipeline (Trigger.dev `sales-video-pipeline` task) carries a `dify` metadata block only: `provider`, `base_url`, `workflow_url`, `configured_groups`, `missing_groups`, `secret_values_in_payload`. n8n is decommissioned (WW-EVENT); the orchestrator is OpenClaw.
