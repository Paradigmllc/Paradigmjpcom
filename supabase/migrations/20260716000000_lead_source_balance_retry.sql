-- Reclaim official Tier 3 SME records whose offer-fit review stopped only
-- because DeepSeek returned the exact Insufficient Balance provider error.
-- Reclaiming does not approve the record: the current deterministic homepage,
-- product, identity, SMB and form gates must all run again from scratch.
    ...
    WHERE source_record.country_code = p_country_code
      ...
      AND NOT EXISTS (
        ...
        WHERE prior_item.domain = source_record.domain
          ...
          AND NOT (
            source_config.trust_tier >= 3
            AND source_record.is_sme = true
            AND prior_item.status = 'review_required'
            AND (
              prior_item.quality_reasons = ARRAY['japan_entry_offer_fit_missing']::text[]
              OR (
                prior_item.quality_reasons = ARRAY['ai_evidence_review_failed']::text[]
                AND prior_item.quality_gate->'aiReview'->>'mode' = 'offer_fit'
                AND COALESCE((prior_item.quality_gate->'aiReview'->>'confidence')::numeric, 0) >= 0.90
                AND jsonb_typeof(prior_item.quality_gate->'aiReview'->'evidenceQuotes') = 'array'
                AND jsonb_array_length(prior_item.quality_gate->'aiReview'->'evidenceQuotes') >= 2
                AND jsonb_typeof(prior_item.quality_gate->'aiReview'->'riskFlags') = 'array'
                AND jsonb_array_length(prior_item.quality_gate->'aiReview'->'riskFlags') = 0
              )
              OR (
                prior_item.quality_reasons = ARRAY['ai_evidence_review_failed']::text[]
                AND prior_item.quality_gate->'aiReview'->>'mode' = 'offer_fit'
                AND prior_item.quality_gate->'aiReview'->>'model' = 'deepseek-v4-pro'
                AND prior_item.quality_gate->'aiReview'->>'passed' = 'false'
                AND COALESCE((prior_item.quality_gate->'aiReview'->>'confidence')::numeric, 0) = 0
                AND prior_item.quality_gate->'aiReview'->>'error' LIKE '%Insufficient Balance%'
                AND prior_item.quality_gate->'aiReview'->'riskFlags' = '["review_failed"]'::jsonb
                AND prior_item.quality_gate->'aiReview'->'evidenceQuotes' = '[]'::jsonb
              )
            )
          )
      )
...
COMMENT ON FUNCTION public.sales_claim_lead_source_records(text, uuid[], integer) IS
  'Claims fresh evidence-bearing records and permits strict revalidation after grounded product reviews or the exact DeepSeek balance outage.';
