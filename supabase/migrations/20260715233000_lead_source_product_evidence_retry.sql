-- Extend the official-SMB retry lane to candidates already adjudicated by
-- DeepSeek with grounded, risk-free product evidence at calibrated confidence.
-- Generic SMB adjudication remains at 96%; this exception is limited to Tier 3
-- records carrying an authoritative SME flag.
...
COMMENT ON FUNCTION public.sales_claim_lead_source_records(text, uuid[], integer) IS
  'Claims fresh evidence-bearing records and permits retries for official-SMB product-fit-only or grounded product-evidence reviews.';
