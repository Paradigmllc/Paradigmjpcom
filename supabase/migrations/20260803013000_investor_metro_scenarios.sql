        ...
        jsonb_build_object(
          ...
          'paragraphs', jsonb_build_array(
            format(
              ...
              mandate, market_preview ->> 'region', lower(strategy_label), lower(profile_label)
            ),
            format(
              'The official evidence scope for %s is: %s The first two published decision anchors are %s and %s. They describe market context, not the value of a parcel or building. For a %s pursuing %s, every benchmark must therefore be bridged to achieved leases, completed transactions and failed listings matched for tenure, legal use, station access, construction, age, size and condition. The committee should write the disconfirming observation in advance and reject the thesis when asset evidence falls outside the documented range instead of changing the narrative after preferred pricing has been selected. In this %s %s file for a %s, the evidence bridge is a closing gate: unresolved differences remain explicit adjustments or reasons to decline.',
              market_preview ->> 'region', market_payload #>> '{marketEvidence,scope}',
              market_payload #>> '{keyFacts,0,value}', market_payload #>> '{keyFacts,1,value}',
              profile_label, lower(strategy_objective), market_preview ->> 'region', lower(strategy_label), lower(profile_label)
            )
          ),
        ...
        jsonb_build_object(
          ...
          'paragraphs', jsonb_build_array(
            format(
              ...
              market_payload #>> '{risks,0,diligenceAction}', breaks_when, market_preview ->> 'region', lower(strategy_label), lower(profile_label)
            ),
            format(
              'Ownership for the %s strategy adds a second control layer for the %s investor. %s Reconcile registered rights, approved use, legal access, seismic and equipment history, repair records, reserves, leases, arrears, insurance and tax coordination to the actual asset and operator. Export national and municipal hazard evidence for the parcel, identify shared river, coastal, slope, transport, employer and utility dependencies, and test downtime and emergency response. The scenario passes only when the Japan-side operating chain is named, contracted and capable of preserving evidence for the %s mandate throughout the hold. In the %s market, that operating chain is assessed specifically for the %s strategy and %s governance path, including who can produce each record after acquisition.',
              lower(strategy_label), lower(profile_label), governance_constraint, lower(profile_label), market_preview ->> 'region', lower(strategy_label), lower(profile_label)
            )
          ),
          'sourceIds', primary_source_ids
        ),
