-- Reopen only the historical Tier 3 official-SMB candidates that were stopped
-- solely because the old deterministic gate recognized EC/SaaS markers but not
-- evidence-bearing B2B, DeepTech, or physical product companies.
    ...
    WHERE source_record.country_code = p_country_code
      ...
      AND NOT EXISTS (
        ...
        WHERE prior_item.domain = source_record.domain
          ...
          AND NOT (
            prior_item.status = 'review_required'
            AND prior_item.quality_reasons = ARRAY['japan_entry_offer_fit_missing']::text[]
            AND source_config.trust_tier >= 3
...
COMMENT ON FUNCTION public.sales_claim_lead_source_records(text, uuid[], integer) IS
  'Claims fresh evidence-bearing records and permits one retry for historical official-SMB product-fit-only reviews.';
